  ...
  persist(
    (set, get) => ({
      ...
      residentConfirmDelivery: (orderId) => {
        const order = get().marketplaceOrders.find(
          (o) => o.id === orderId
        );
            ...
            o.id === orderId
              ? {
                ...
                timeline: [
                  ...o.timeline,
                  {
                    action: 'Delivery Confirmed by Resident',
                    at: now(),
                  },
                ],
              }
        ...
        // Add vendor earnings entry
        if (order) {
          get().addEarningsEntry({
            vendorId: order.vendorId || 'ven1',
            storeId: order.storeId || 'store1',
      ...
      addEarningsEntry: (entry) => {
        const record = {
          ...
          source: 'resident_confirmed',
        };

        set((s) => ({
          vendorEarnings: [record, ...s.vendorEarnings],
        }));
...

    {
      ...
      migrate: (persistedState, version) => {
        return {
          ...persistedState,
          marketplaceOrders: SEED_ORDERS,
          vendorEarnings: persistedState.vendorEarnings || [],
          maintenanceRequests: SEED_MAINTENANCE,
