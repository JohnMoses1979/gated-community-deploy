...
const mergeDocuments = (saved = []) => {
  if (!Array.isArray(saved) || saved.length === 0) return initialDocs;
  const savedById = new Map(saved.map((doc) => [String(doc.id), doc]));
  const mergedDefaults = initialDocs.map((doc) => ({
    ...doc,
    ...(savedById.get(String(doc.id)) || {}),
  }));
  const defaultIds = new Set(initialDocs.map((doc) => String(doc.id)));
  const customDocs = saved.filter((doc) => !defaultIds.has(String(doc.id)));
  return [...customDocs, ...mergedDefaults];
};
...
export default function BuilderComplianceTracking({ navigation, route }) {
  ...
  const fetchComplianceDocuments = useBuilderStore((s) => s.fetchComplianceDocuments);
  const updateComplianceDocuments = useBuilderStore((s) => s.updateComplianceDocuments);

  const selectedProject = useMemo(
    () => projects.find((p) => String(p.id) === String(routeProject?.id)) || routeProject,
    [projects, routeProject]
  );

  const [documents, setDocuments] = useState(() =>
    mergeDocuments(selectedProject?.complianceDocuments)
  );
  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      const result = await fetchComplianceDocuments(selectedProject.id);
      if (result?.success) {
        setDocuments(mergeDocuments(result.data));
      } else {
        setDocuments(mergeDocuments(selectedProject?.complianceDocuments));
      }
    };
  ...
  const saveDocuments = async (nextDocuments) => {
    ...
    if (selectedProject?.id) {
      const result = await updateComplianceDocuments(selectedProject.id, nextDocuments);
      if (!result?.success) {
        console.warn("Compliance documents were updated locally but backend save failed:", result?.message);
      }
    }
  };
