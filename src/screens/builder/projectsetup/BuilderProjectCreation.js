...
export default function ProjectCreation({ navigation, route }) {
  ...
  const createBuilderProject = useBuilderStore((s) => s.createProject);
  ...
  const createProject = async () => {
    ...
    if (!result.success) {
      showMessage(result.message || "Project submission failed.", "error");
      return;
    }

    showMessage(
      "Project submitted to Super Admin for approval. You can add units once approved.",
      "success"
    );
    await fetchProjects();
