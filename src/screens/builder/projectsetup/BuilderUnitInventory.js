...
export default function UnitInventory({ navigation, route }) {
  ...
  const addUnitToProject = useBuilderStore((s) => s.addUnitToProject);
  const updateUnitInProject = useBuilderStore((s) => s.updateUnitInProject);
  const deleteUnitFromProject = useBuilderStore((s) => s.deleteUnitFromProject);
  ...
  const units = selectedProject?.units?.length
    ...
    : (selectedProject?.towers || []).flatMap((projectTower) => projectTower.units || []);
  ...
  const availableUnits = units.filter((u) => String(u.status).toLowerCase() === "available").length;
  const holdUnits = units.filter((u) => String(u.status).toLowerCase() === "hold").length;
  ...
  const saveUnit = async () => {
    ...
    const result = editingId
      ? await updateUnitInProject(selectedProject.id, editingId, payload)
      : await addUnitToProject(selectedProject.id, payload);

    if (!result?.success) {
      showMessage(result?.message || "Unit save failed.", "error");
      return;
    }

    showMessage(editingId ? "Unit updated successfully." : "Unit created and linked to project setup.", "success");
    resetForm();
  };
    ...
    Alert.alert("Delete Unit", "Are you sure you want to remove this unit?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          const result = await deleteUnitFromProject(selectedProject.id, id);
          if (!result?.success) {
            showMessage(result?.message || "Unit delete failed.", "error");
            return;
          }
