...
import {
  ...
  TouchableOpacity, StatusBar, TextInput, Modal, Platform, Alert,
} from "react-native";
import { Ionicons, MaterialCommunityIcons } from "@expo/vector-icons";
// ADD:
import { useBuilderStore } from '../../store/builderStore';
// ADD alongside the existing useBuilderStore import:
...
const VISIT_TABS = ["All", "Pending", "Approved", "Completed", "Rejected"];
...
const statusConfig = {
  ...
  Completed: { label: "Completed", color: C.blue, bg: C.blueLight },
...
export default function BuilderVisitBooking({ navigation, route }) {
  ...
  // ADD this line to the existing store destructure:
  ...
  const handleApprove = async (id) => {
    const result = await approveVisitRequest(
      id,
  ...
  const handleReject = async (id) => {
    const result = await rejectVisitRequest(
      id,
    ...
    <SafeAreaView style={sc.safe}>
      ...
      <View style={sc.root}>
        <View style={sc.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={sc.headerTitle}>Guest Visits</Text>
            <Text style={sc.headerSub}>Customer visit requests · Approvals</Text>
          </View>
        </View>
