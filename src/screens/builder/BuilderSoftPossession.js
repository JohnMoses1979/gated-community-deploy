...
export default function SoftPossessionBuilderScreen({ navigation, route }) {
  ...
  const fetchFlatBookings = useBuilderStore((s) => s.fetchFlatBookings);
  const getPaymentPercentage = useBuilderStore((s) => s.getPaymentPercentage);
  const updatePossessionStatus = useBuilderStore((s) => s.updatePossessionStatus);
  const completePayment = useBuilderStore((s) => s.completePayment);
  // WITH:
  ...
  // Derive soft possession candidates from real approved bookings
  // A booking is a candidate once unit is BOOKED or SOLD (payment has happened)
  const requests = useMemo(() => {
    return flatBookingRequests
      .filter((b) => b._backendStatus === 'APPROVED')
      .map((b) => {
        const payPct = getPaymentPercentage(b);
        return {
          id: String(b.id),
          customerName: b.customerName || b.guestName || 'Customer',
          unitId: b.unitNo || b.unitNumber || '-',
          tower: b.unit?.tower || '-',
          floor: b.unit?.floor || '-',
          projectName: b.projectName || '-',
          unitType: b.unitType || 'Flat',
          paymentPercent: payPct,
          eligibilityTriggered: payPct >= 80,
          // softPossessionStatus comes from backend UnitBooking.softPossessionStatus
          builderReviewed: b.softPossessionStatus === true,
          possessionLetterIssued: !!b.possessionLetterUrl,
          interiorPermissionGranted: b.softPossessionStatus === true,
          inspectionScheduled: false, // local only until backend field is added
          interiorWorkStatus: payPct >= 100 ? 'Completed' : payPct >= 80 ? 'In Progress' : 'Not Started',
          finalPossessionReady: payPct >= 100 && b.softPossessionStatus === true,
          stage: payPct >= 100 ? 'Ready For Final Possession'
            : b.softPossessionStatus ? 'Interior Work In Progress'
              : payPct >= 80 ? 'Eligibility Triggered' : 'Pending',
          remarks: b.softPossessionStatus
            ? 'Soft possession granted by builder.'
            : payPct >= 80
              ? 'Customer crossed payment threshold. Awaiting builder review.'
              : 'Payment below threshold.',
          timeline: [],
          // keep original booking id for API calls
          _bookingId: b.id,
        };
      });
  }, [flatBookingRequests]);
  ...
  const selectedRequest = useMemo(() => {
    if (selectedId) return requests.find((item) => item.id === selectedId) || requests[0] || null;
  ...
  const markReviewed = async () => {
    ...
    const result = await updatePossessionStatus(selectedRequest._bookingId, true, '');
    if (!result.success) {
      Alert.alert('Error', result.message || 'Update failed.');
      return;
    }
    await fetchFlatBookings();
    Alert.alert('Updated', 'Eligibility review marked and soft possession granted.');
  };
  ...
  const issueLetter = async () => {
    if (!selectedRequest) return;
    if (!selectedRequest.builderReviewed) {
      Alert.alert('Pending step', 'Mark eligibility reviewed first.');
      return;
    }
    ...
    // The possession letter URL would come from a document upload in production.
    // For now we mark possession with a placeholder URL.
    const result = await updatePossessionStatus(
      selectedRequest._bookingId,
      true,
      `LETTER-${selectedRequest._bookingId}-${Date.now()}`
    );
    if (!result.success) {
      Alert.alert('Error', result.message || 'Letter issue failed.');
      return;
    }
    await fetchFlatBookings();
  ...
  const grantPermission = async () => {
    ...
    const result = await updatePossessionStatus(selectedRequest._bookingId, true, selectedRequest.possessionLetterIssued ? undefined : '');
    if (!result.success) {
      Alert.alert('Error', result.message || 'Permission update failed.');
      return;
    }
  ...
  const confirmInspection = () => {
    ...
    Alert.alert(
      'Inspection Not Persisted',
      'The backend does not yet expose an inspection schedule field. Add a DB-backed inspection endpoint before using this as an enterprise lifecycle step.'
    );
  };

  // REPLACE markReadyForFinal:
  const markReadyForFinal = async () => {
    if (!selectedRequest) return;
    if (!selectedRequest.possessionLetterIssued) {
      Alert.alert('Pending step', 'Issue the soft possession letter first.');
      return;
    }
    if (selectedRequest.paymentPercent < 100) {
      Alert.alert('Payment Incomplete', 'Full payment required before final possession.');
      return;
    }
    Alert.alert(
      'Confirm Final Possession',
      'This will mark the unit as SOLD. Are you sure?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Confirm',
          onPress: async () => {
            const result = await completePayment(selectedRequest._bookingId);
            if (!result.success) {
              Alert.alert('Error', result.message || 'Could not complete payment/SOLD transition.');
              return;
            }
            await fetchFlatBookings();
            Alert.alert('Done', 'Unit marked as SOLD and ready for final possession.');
          },
        },
      ]
    );
