...
export default function BuilderProjectSetup({ navigation, route }) {
  const builderProjects = useBuilderStore((s) => s.projects);
  ...
  const units = selectedProject?.units?.length
    ? selectedProject.units
    : (selectedProject?.towers || []).flatMap((tower) => tower.units || []);
  const documents = selectedProject?.complianceDocuments || [];
  const isProjectApproved = selectedProject?.approvalStatus === "Approved";
  const uploadedDocs = documents.filter((d) => d.file || d.status === "Uploaded" || d.status === "Verified").length;
  const hasUnits = units.length > 0;
  const hasUploadedDocs = uploadedDocs > 0;
  const setupReadyForCustomers = isProjectApproved && hasUnits && hasUploadedDocs;

  const setupProgress = useMemo(() => {
    let score = 0;

    if (selectedProject?.projectName || selectedProject?.name) score += 33;
    if (isProjectApproved && hasUnits) score += 33;
    if (isProjectApproved && hasUploadedDocs) score += 34;

    return score;
  }, [selectedProject, hasUnits, hasUploadedDocs, isProjectApproved]);
        ...
        >
          {approvedFirstProjects.length === 0 ? (
            ...
            <>
              ...
              {(hasUploadedDocs || setupReadyForCustomers) && (
                <>
                  <StepCard
                    number="4"
                    icon="send-outline"
                    title="Send to Customers"
                    description="Publish this project so logged-in customers can view units, documents, visit booking and flat booking."
                    completed={setupReadyForCustomers}
                    locked={!setupReadyForCustomers}
                    lockReason="Add at least one unit and upload compliance documents first."
                    color={COLORS.green}
                  ...
                  <View style={styles.visibilityCard}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.visibilityTitle}>Customer Visibility</Text>
                      <Text style={styles.visibilitySub}>
                        Approved LIVE projects with units and uploaded compliance documents are available in the Property Buyer section.
                      </Text>
                    </View>

                    <View style={[
                      styles.visibilityBadge,
                      setupReadyForCustomers && styles.visibilityBadgeActive,
                    ]}>
                      <Ionicons
                        name={setupReadyForCustomers ? "checkmark-circle-outline" : "time-outline"}
                        size={16}
                        color={setupReadyForCustomers ? COLORS.white : COLORS.navy}
                      />
                      <Text style={[
                        styles.visibilityBadgeText,
                        setupReadyForCustomers && styles.visibilityBadgeTextActive,
                      ]}>
                        {setupReadyForCustomers ? "Live" : "Pending"}
                      </Text>
                    </View>
                  </View>
                </>
