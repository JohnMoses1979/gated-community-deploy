...
export default function ConstructionTrackingScreen({ navigation, route }) {
  ...
  const fetchProjects = useBuilderStore((s) => s.fetchProjects);
  const [milestones, setMilestones] = useState([]);
  const [updates, setUpdates] = useState([]);
  const [selectedProjectId, setSelectedProjectId] = useState(route?.params?.project?.id || null);
  ...
  useEffect(() => {
    ...
    async function loadConstruction() {
      ...
      const milestoneResp = await fetchProjectMilestones(token, selectedProject.id);
      const liveMilestones = milestoneResp.response && milestoneResp.response.ok && Array.isArray(milestoneResp.data)
        ? milestoneResp.data
        : [];
      const mappedMilestones = liveMilestones.map((item) => ({
        id: item.id,
        name: item.title,
        expectedDate: item.expectedCompletionDate || '',
        completion: Number(item.percentageCompletion || 0),
        status: item.status === 'COMPLETED' ? 'Completed' : item.status === 'IN_PROGRESS' ? 'In Progress' : 'Pending',
      }));
      const updateEntries = await Promise.all(
        liveMilestones.map(async (milestone) => {
          const updateResp = await fetchMilestoneUpdates(token, milestone.id);
          const milestoneUpdates = updateResp.response && updateResp.response.ok && Array.isArray(updateResp.data)
            ? updateResp.data
            : [];
          return milestoneUpdates.map((item) => ({
            id: item.id,
            milestoneId: milestone.id,
            milestone: milestone.title,
            tower: '',
            phase: '',
            completion: Number(milestone.percentageCompletion || 0),
            expectedDate: milestone.expectedCompletionDate || '',
            note: item.description,
            delay: false,
            photoLink: JSON.parse(item.photoUrlsJson || '[]')[0] || '',
            videoLink: JSON.parse(item.videoUrlsJson || '[]')[0] || '',
            sentToCustomers: true,
            createdAt: item.updatedDate || '',
          }));
        })
      );
      if (mounted) {
        setMilestones(mappedMilestones);
        setUpdates(updateEntries.flat().sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt))));
      }
    }
    loadConstruction();
    return () => {
  ...
  const handleSaveProgressUpdate = async () => {
    if (!selectedProject?.id) {
      Alert.alert('Project Required', 'Create or select a builder project first.');
      return;
    }
    ...
    const token = useAuthStore.getState().token;
    const existingMilestone = milestones.find((item) => item.name === selectedMilestone);
    const milestonePayload = {
      title: selectedMilestone,
      percentageCompletion: progressValue,
      expectedCompletionDate: expectedDate,
    };

    const milestoneResult = existingMilestone
      ? await updateMilestone(token, existingMilestone.id, milestonePayload)
      : await createMilestone(token, selectedProject.id, milestonePayload);

    if (!milestoneResult.response || !milestoneResult.response.ok) {
      Alert.alert('Error', milestoneResult.data?.message || 'Milestone save failed.');
      return;
    }

    const savedMilestone = milestoneResult.data;
    const updateResult = await addConstructionUpdate(token, savedMilestone.id, {
      description: `${selectedTower} · ${selectedPhase}: ${note.trim() || 'Progress updated by builder.'}${delayReason.trim() ? ` Delay: ${delayReason.trim()}` : ''}`,
      photoUrlsJson: JSON.stringify(photoLink.trim() ? [photoLink.trim()] : []),
      videoUrlsJson: JSON.stringify(videoLink.trim() ? [videoLink.trim()] : []),
    });

    if (!updateResult.response || !updateResult.response.ok) {
      Alert.alert('Error', updateResult.data?.message || 'Progress update save failed.');
      return;
    }
    ...
    setUpdates((prev) => [{
      ...
      milestone: savedMilestone.title,
      tower: selectedTower,
      phase: selectedPhase,
      completion: progressValue,
      expectedDate,
      note: updateResult.data.description,
      delay: delayReason.trim().length > 0,
      photoLink: photoLink.trim(),
      videoLink: videoLink.trim(),
      sentToCustomers: notifyCustomers,
      createdAt: updateResult.data.updatedDate || 'Just now',
    }, ...prev]);

    const refreshed = await fetchProjectMilestones(token, selectedProject.id);
    if (refreshed.response && refreshed.response.ok && Array.isArray(refreshed.data)) {
      setMilestones(refreshed.data.map((item) => ({
        id: item.id,
        name: item.title,
        expectedDate: item.expectedCompletionDate || '',
        completion: Number(item.percentageCompletion || 0),
    ...
    Alert.alert(
      'Progress Updated',
      notifyCustomers
        ? 'Progress updated and stored in the backend.'
  ...
  const handleSendCustomerNotification = item => {
    Alert.alert(
      'Update Available',
      `${item.milestone} is already stored in the backend and visible through the authenticated construction read endpoints.`
    );
  };
