...
function VerificationBanner({ user, navigation }) {
  const vs = user?.verificationStatus || "not_submitted";
  if (vs === "approved") return null;

  const isPending = vs === "pending" || vs === "pending_approval";

  return (
    <TouchableOpacity
      style={[
        styles.verificationCard,
        isPending ? styles.pendingBorder : styles.notSubmittedBorder,
      ]}
      activeOpacity={0.85}
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        <Text style={styles.verificationTitle}>
          {isPending ? "Verification Pending" : "Profile Not Verified"}
        </Text>
        <Text style={styles.verificationSub}>
          {isPending
            ? "SuperAdmin is reviewing your documents."
            : "Please upload your RERA & Company docs to create projects."}
        </Text>
      </View>
      <Ionicons name="chevron-forward" size={18} color={COLORS.sub} />
    </TouchableOpacity>
...
export default function BuilderDashboard({ navigation, route }) {
  ...
  const builderId = user?.id || route?.params?.builderId;

  const fetchDashboardStats = useBuilderStore((s) => s.fetchDashboardStats);
  // ADD THIS (reads live verificationStatus from registeredUsers):
  ...
  const dashboardStats = useBuilderStore((s) => s.dashboardStats);
  const isLoading = useBuilderStore((s) => s.isLoading);
  const projects = dashboardStats?.projects || [];

  // Re-fetch dashboard stats and user status every time this screen gains focus
  // so that any SuperAdmin project or profile approval is immediately visible.
  useFocusEffect(
    useCallback(() => {
      fetchDashboardStats();
      // verificationStatus auto-updates from AuthStore.registeredUsers — no extra call needed
    }, [fetchDashboardStats])
  );
  ...
  const fallbackProject = {
    ...
    name: "No Project Yet",
    projectName: "No Project Yet",
    location: "Add your first project to get started",
    towers: [],
  };

  const allProjects = projects.length > 0 ? projects : [fallbackProject];

  const [selectedProjectId, setSelectedProjectId] = useState(allProjects[0]?.id);

  // If projects update, ensure selection is valid
  useEffect(() => {
  ...
  const unreadCount = 0; // Notification logic can be added later
        ...
        >
          ...
          <View style={styles.heroCard}>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.85}
              onPress={() => {
                if (selectedProject.id === "demo-project") {
                  navigation.navigate("BuilderProjectSetup");
                } else {
                  navigation.navigate("BuilderProjectSetup", { project: selectedProject });
                }
              }}
            >
              <Text style={styles.heroBtnText}>Manage</Text>
          ...
          <View style={styles.statsGrid}>
            ...
            <StatCard
              icon="business-outline"
              label="Towers"
              value={dashboardStats?.totalTowers || 0}
              color={COLORS.orange}
              onPress={() => navigation.navigate("BuilderProjectSetup")}
            />
            ...
            <StatCard
              icon="wallet-outline"
              label="Collected"
              value={formatMoney(0)} // TODO: Bind to Payment API later
              color={COLORS.green}
              onPress={() => { }}
            />
          </View>
          ...
          <QuickAction
            icon="calendar-outline"
            title="Guest Visits"
            subtitle="Approve site visit requests"
            color={COLORS.orange}
            onPress={() =>
              navigation.navigate("BuilderVisitBooking", {
                project: selectedProject,
          ...
          <QuickAction
            icon="document-text-outline"
            title="Guest Bookings"
            subtitle="Review customer flat bookings"
            color={COLORS.gold}
            onPress={() =>
              navigation.navigate("BuilderUnitBooking", {
                project: selectedProject,
...
const styles = StyleSheet.create({
  ...
  notSubmittedBorder: { borderColor: COLORS.red + "44" },
  pendingBorder: { borderColor: COLORS.orange + "44" },
  ...
  verificationTitle: { fontSize: 16, fontWeight: "800", color: COLORS.text, marginBottom: 2 },
  verificationSub: { fontSize: 12, color: COLORS.sub, lineHeight: 18 },
});