...
// ADD at the top of BuilderPaymentSchedule.js with other imports:
import { recordManualCollection } from '../../api/builderApi';
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.actionRow}>
        ...
        {percent >= 100 && booking._backendStatus === 'APPROVED' && (
          <ActionBtn
            icon="checkmark-done-circle-outline"
            label="Mark SOLD"
            primary
            onPress={() => onMarkSold(booking)}
          />
        )}
      </View>
    </View>
...
export default function BuilderPaymentSchedule({ navigation }) {
  ...
  const getBookingTotalAmount = useBuilderStore((s) => s.getBookingTotalAmount);

  // ADD to the existing store destructure:
  const fetchFlatBookings = useBuilderStore((s) => s.fetchFlatBookings);
  const fetchInstallmentsForBooking = useBuilderStore((s) => s.fetchInstallmentsForBooking);
  ...
  // ADD:
  const [installmentMap, setInstallmentMap] = useState({}); // bookingId → installments[]
    ...
    async function loadInstallments() {
      const entries = await Promise.all(
        approvedBookings.map(async (booking) => {
          const result = await fetchInstallmentsForBooking(booking.id);
          return [booking.id, result.success ? result.data : []];
        })
      );
      if (mounted) {
  ...
  }, [approvedBookings, fetchInstallmentsForBooking]);

  const getPaid = (b) => {
    const installments = installmentMap[b.id] || [];
  ...
  const getPaymentPercentage = (b) => pct(getPaid(b), getTotal(b));
  ...
  const filteredList = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((b) =>
        `${b.customerName || b.guestName || ""} ${b.phone || b.customerPhone || ""} ${b.projectName || ""
  ...
  const submitPayment = async ({ amount, method }) => {
    ...
    try {
      const { response, data } = await recordManualCollection(
        token,
        selBooking.id,
        amount,
        method ? `Manual Collection - ${method}` : ""
      );

      if (!response || !response.ok) {
        Alert.alert("Error", data?.message || "Payment failed. Please try again.");
        return;
      }
      ...
      const refreshed = await fetchInstallmentsForBooking(selBooking.id);
      if (refreshed.success) {
        setInstallmentMap((prev) => ({ ...prev, [selBooking.id]: refreshed.data }));
      }
  ...
  // ADD after sendFinalReceipt:
  const handleMarkSold = async (booking) => {
    if (!booking?.id) return;
    Alert.alert(
      'Mark as SOLD',
      `Confirm full payment received for ${booking.customerName || booking.guestName}? This will mark the unit as SOLD.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (!result.success) {
              Alert.alert('Error', result.message || 'Could not mark as SOLD.');
              return;
            }
            Alert.alert('Done', 'Unit marked as SOLD. Possession is now available.');
          },
        },
      ]
    );
