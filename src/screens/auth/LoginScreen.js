...
function LoginContent({ navigation }) {
  ...
  const authenticateCustomerProfile = useAuthStore((s) => s.authenticateCustomerProfile);
  ...
  const handleCustVerifyOtp = async () => {
    ...
    if (existing) {
      await authenticateCustomerProfile(existing);
    } else {
      setCustStep(3);
    }
  };

  const handleCustCreateProfile = async () => {
    if (!custName.trim()) {
    ...
    await authenticateCustomerProfile(customer);
  };
              ...
              {topTab === "staff" && (
                <>
                  ...
                  <View style={s.roleGrid}>
                    {["Resident", "Vendor", "Guard", "Admin", "Builder", "Super Admin", "Customer"].map(
                      (role) => (
                        <TouchableOpacity
                          key={role}
                          activeOpacity={0.85}
