...
export default function ProjectRequestDetails({ navigation, route }) {
  ...
  const handleApprove = async () => {
    ...
    const result = await approveProjectRequest(project.id, 'Approved by Super Admin');
    setIsSubmitting(false);

    if (!result?.success) {
      Alert.alert('Approval Failed', result?.message || 'Project approval failed.');
      return;
    }
  ...
  const handleReject = async () => {
    ...
    setIsSubmitting(true);
    const result = await rejectProjectRequest(project.id, remark.trim());
    setIsSubmitting(false);

    if (!result?.success) {
      Alert.alert('Reject Failed', result?.message || 'Project rejection failed.');
      return;
    }
      ...
      >
        ...
        <View style={styles.actionRow}>
          <TouchableOpacity
            ...
            onPress={isSubmitting ? undefined : handleApprove}
          >
          ...
          <TouchableOpacity
            ...
            onPress={isSubmitting ? undefined : handleReject}
          >
