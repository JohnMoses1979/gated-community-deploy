...
const getBuilderStore = () => require("../../store/builderStore").default.getState();
...
const projectNotificationId = (projectId) => `project-request-${projectId}`;
...
export const SocietyProvider = ({ children }) => {
  ...
  const syncProjectNotifications = (projects = []) => {
    const pending = projects.filter((p) => p.approvalStatus === "Pending");

    setNotifications((prev) => {
      const existingIds = new Set((prev || []).map((n) => n.id));
      const nextNotifications = pending
        .filter((project) => !existingIds.has(projectNotificationId(project.id)))
        .map((project) => ({
          id: projectNotificationId(project.id),
          title: "New Project Submitted",
          message: `Builder "${project.builderName || "Builder"}" submitted project "${project.projectName || project.name}" for approval.`,
          builderId: project.builderId || null,
          type: "info",
          category: "project",
          targetRole: "superadmin",
          screen: "ProjectRequestDetails",
          screenParams: { projectRequestId: project.id },
          read: false,
          time: new Date(project.requestedAt || project.createdAt || Date.now()).toLocaleString(),
          createdAt: project.requestedAt || project.createdAt || new Date().toISOString(),
        }));

      return [...nextNotifications, ...prev];
    });
  };
  ...
  const refreshProjectRequests = async () => {
    try {
      const data =
        typeof getAuth().fetchAdminProjects === "function"
          ? await getAuth().fetchAdminProjects()
          : await getAuth().fetchPendingProjects();

      const normalized = (Array.isArray(data) ? data : []).map((p) =>
        normalizeProjectPayload(p, p.approvalStatus || "Pending")
      );

      setProjectRequests(normalized);
      setBuilderProjects(normalized);
      syncProjectNotifications(normalized);
      return { success: true, data: normalized };
    } catch (e) {
      console.warn("Failed to load builder projects", e);
      return { success: false, message: "Failed to load builder projects" };
    }
  };
  ...
  const approveProjectRequest = async (projectId, reviewMessage = "Approved by Super Admin") => {
    try {
      const { success, data, message } = await getAuth().approveProject(projectId, true, reviewMessage);
      if (!success) return { success: false, message };

      const approved = normalizeProjectPayload(data, data.getApprovalStatus ? data.getApprovalStatus() : data.approvalStatus || "Approved");

      setProjectRequests((prev) => prev.map((p) => (String(p.id) === String(projectId) ? approved : p)));
      setBuilderProjects((prev) => prev.map((p) => (String(p.id) === String(projectId) ? approved : p)));
      setNotifications((prev) =>
        (prev || []).map((n) =>
          n.id === projectNotificationId(projectId)
            ? { ...n, read: true, approved: true }
            : n
        )
      );

      try { getBuilderStore().fetchProjects && getBuilderStore().fetchProjects(); } catch (e) {}
      try {
        const customerStore = require("../../store/CustomerStore").default;
        customerStore.getState().fetchProjects && customerStore.getState().fetchProjects();
      } catch (e) {}

      addNotification({
        title: "✅ Project Approved",
        message: `Your project "${approved.projectName || approved.name}" has been approved.`,
        builderId: approved.builderId || null,
        type: "success",
        category: "project",
      });
      await refreshProjectRequests();
      return { success: true, data: approved };
    } catch (e) {
      console.error('approveProjectRequest error', e);
      return { success: false, message: "Project approval failed" };
    }
  };

  const rejectProjectRequest = async (projectId, reviewMessage = "Rejected by Super Admin") => {
    try {
      const { success, data, message } = await getAuth().approveProject(projectId, false, reviewMessage);
      if (!success) return { success: false, message };

      const rejected = normalizeProjectPayload(data, data.approvalStatus || "Rejected");

      setProjectRequests((prev) => prev.map((p) => (String(p.id) === String(projectId) ? rejected : p)));
      setBuilderProjects((prev) => prev.map((p) => (String(p.id) === String(projectId) ? rejected : p)));
      setNotifications((prev) =>
        (prev || []).map((n) =>
          n.id === projectNotificationId(projectId)
      ...
      addNotification({
        title: "❌ Project Rejected",
        message: `Your project "${rejected.projectName || rejected.name || 'Unnamed'}" was rejected. Reason: ${reviewMessage}`,
        builderId: rejected.builderId || null,
        type: "danger",
        category: "project",
      });
      ...
      console.error('rejectProjectRequest error', e);
      return { success: false, message: "Project rejection failed" };
    }
  };
