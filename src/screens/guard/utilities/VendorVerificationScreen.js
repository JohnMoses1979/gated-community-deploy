 ...
 *
 * SECURITY FIX (merged):
 *  - appStore replaced with maintenanceStore (guardVerifyGateOTP)
 *  - Token from useAuthStore(s => s.token)
 *  - guardId never sent — backend reads from JWT
 *  - Ionicons replaced with emoji fallback where Ionicons unavailable
 *
 * Flow:
 *   Admin approves work start → vendorGateOTP generated → status = approved_to_start
 *   Guard enters OTP → POST /maintenance/verify-gate-otp → status = work_in_progress
 */
...
export default function VendorVerificationScreen({ navigation }) {
  ...
  const fetchGateRequests      = useMaintenanceStore(s => s.fetchGateRequests);
  const guardVerifyGateOTP     = useMaintenanceStore(s => s.guardVerifyGateOTP);
  ...
  const loadRequests = useCallback(async (isRefresh = false) => {
    ...
    await fetchGateRequests(token);
    if (isRefresh) setRefreshing(false);
  }, [token, fetchGateRequests]);

  useEffect(() => { loadRequests(); }, [loadRequests]);

  // Approved_to_start = waiting at gate for OTP
  const pending = requests.filter(r =>
    r.status === 'approved_to_start' && r.assignedVendorId &&
    (!search ||
      r.assignedVendorName?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  // work_in_progress = vendor already inside
  const inside = requests.filter(r =>
    r.status === 'work_in_progress' && r.assignedVendorId &&
    (!search ||
      r.assignedVendorName?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const handleVerify = async (requestId, otp) => {
    setVerifying(prev => ({ ...prev, [requestId]: true }));
    const result = await guardVerifyGateOTP(token, otp);
    setVerifying(prev => ({ ...prev, [requestId]: false }));
    ...
    if (result.ok) {
      ...
      Alert.alert(
        '✅ Entry Allowed',
        `${req?.assignedVendorName || 'Vendor'} verified. Work has started at Unit ${req?.unit || '—'}.`
      );
    ...
    return (
      <View style={s.card}>
        <View style={s.cardTop}>
          <View style={s.cardIcon}><Text style={{ fontSize: 24 }}>🔧</Text></View>
          <View style={{ flex: 1 }}>
            <Text style={s.vendorName}>{job.assignedVendorName || 'Vendor'}</Text>
            <Text style={s.jobTitle} numberOfLines={1}>{job.title}</Text>
          </View>
          <View style={[s.badge, { backgroundColor: meta.bg }]}>
            <Text style={[s.badgeText, { color: meta.color }]}>{meta.label}</Text>
        ...
        <View style={s.details}>
          {[
            ...
            ['Unit',     `${job.unit || '—'} · ${job.residentName || '—'}`],
        ...
        {isApproved && (
          <View>
            <View style={s.otpBanner}>
              <Text style={s.otpBannerText}>🔐 Ask vendor for their gate OTP and enter it below</Text>
            </View>
              ...
              <>
                ...
                <View style={s.otpRow}>
                  <TextInput
                    ...
                    value={otp}
                    onChangeText={t =>
                      setOtpInputs(prev => ({ ...prev, [job.id]: t.replace(/\D/g, '').slice(0, 6) }))
                    }
                    placeholder="● ● ● ● ● ●"
                    placeholderTextColor={P.muted}
                    keyboardType="number-pad"
                  ...
                  />
                  <TouchableOpacity
                    style={[s.verifyBtn, (otp.length < 6 || busy) && { opacity: 0.4 }]}
                    onPress={() => handleVerify(job.id, otp)}
        ...
        {isInside && (
          <View style={s.insideRow}>
            ...
            <Text style={s.insideText}>Currently working inside — Work Started</Text>
          </View>
        )}
      </View>
    );
  ...
  const handleManualVerify = async () => {
    if (mOtp.length < 6) return;
    setMBusy(true);
    const result = await guardVerifyGateOTP(token, mOtp);
    setMBusy(false);

    if (result.ok) {
      setMResult('ok');
      Alert.alert('✅ Verified!', `Entry allowed. Job: ${result.request?.id || ''}`);
      setMOtp('');
      loadRequests();
    } else {
      setMResult('fail');
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity
          style={s.manualBtn}
          onPress={() => { setManualModal(true); setMResult(null); setMOtp(''); }}
        >
      ...
      <View style={s.infoBanner}>
        <Text style={s.infoText}>
          Vendors receive a 6-digit OTP when admin approves work to start. Always verify OTP before granting entry.
        </Text>
      </View>
      ...
      <View style={s.tabRow}>
        {[
          ['pending', `Awaiting Entry (${pending.length})`],
      ...
      <View style={s.searchRow}>
        ...
        <TextInput
          style={s.searchInput}
          placeholder="Search vendor, job ID, unit…"
          value={search}
      ...
      {loading && !refreshing ? (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          <ActivityIndicator size="large" color={P.navy} />
        </View>
      ) : (
        <FlatList
          data={displayed}
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              ...
              <Text style={s.emptySub}>
                {tab === 'pending'
                  ? 'Vendors appear here once admin approves work to start.'
                  : 'Verified vendors currently working inside appear here.'}
              </Text>
              <TouchableOpacity style={s.manualLookupBtn} onPress={() => setManualModal(true)}>
                <Text style={{ fontSize: 14 }}>⌨️</Text>
                <Text style={s.manualLookupText}>Manual OTP Entry</Text>
              </TouchableOpacity>
            </View>
          }
        />
      ...
      >
        ...
        >
          <View style={s.modalOverlay}>
            <View style={s.modalCard}>
              <View style={s.modalHeader}>
                <Text style={s.modalTitle}>⌨️ Manual OTP Entry</Text>
                <TouchableOpacity onPress={() => setManualModal(false)}>
                  <Text style={{ fontSize: 22, color: P.sub }}>✕</Text>
                </TouchableOpacity>
              </View>

              <Text style={s.fieldLabel}>Gate OTP</Text>
              <TextInput
                style={[s.fieldInput, { fontSize: 24, fontWeight: '900', letterSpacing: 10, textAlign: 'center' }]}
                value={mOtp}
                onChangeText={t => { setMOtp(t.replace(/\D/g, '').slice(0, 6)); setMResult(null); }}
              ...
              <View style={{ marginTop: 8, backgroundColor: P.amberBg, borderRadius: 10, padding: 10, borderLeftWidth: 3, borderLeftColor: P.amber }}>
                <Text style={{ fontSize: 12, color: '#78350F', fontWeight: '600' }}>
                  💡 The backend matches the OTP to the correct job automatically.
                </Text>
              </View>
              ...
              <TouchableOpacity
                style={[s.modalBtn, (mOtp.length < 6 || mBusy) && { opacity: 0.4 }]}
                onPress={handleManualVerify}
                disabled={mOtp.length < 6 || mBusy}
                activeOpacity={0.85}
              >
                {mBusy
...
const s = StyleSheet.create({
  root:            { flex: 1, backgroundColor: P.bg },
  header:          { backgroundColor: P.navyDark, paddingTop: 40, paddingBottom: 16, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', gap: 14 },
  backBtn:         { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' },
  headerTitle:     { color: '#FFF', fontSize: 18, fontWeight: '900' },
  headerSub:       { color: 'rgba(255,255,255,0.65)', fontSize: 12 },
  manualBtn:       { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center', marginLeft: 'auto' },
  ...
  searchRow:       { flexDirection: 'row', alignItems: 'center', backgroundColor: P.surface, marginHorizontal: 16, marginVertical: 10, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10, borderWidth: 1, borderColor: P.border, gap: 8 },
  searchInput:     { flex: 1, fontSize: 14, color: P.text },
  card:            { backgroundColor: P.surface, borderRadius: 18, padding: 18, marginBottom: 14, borderWidth: 1, borderColor: P.border, elevation: 2 },
  cardTop:         { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 14 },
  cardIcon:        { width: 48, height: 48, borderRadius: 24, backgroundColor: P.bg, alignItems: 'center', justifyContent: 'center' },
  vendorName:      { fontSize: 16, fontWeight: '800', color: P.text },
  jobTitle:        { fontSize: 12, color: P.sub, marginTop: 2 },
  ...
  otpRow:          { flexDirection: 'row', gap: 10 },
  otpInput:        { flex: 1, backgroundColor: P.bg, borderWidth: 1.5, borderColor: P.border, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 13, fontSize: 22, fontWeight: '800', color: P.text, letterSpacing: 8, textAlign: 'center' },
  verifyBtn:       { backgroundColor: P.navy, borderRadius: 12, paddingHorizontal: 18, justifyContent: 'center', alignItems: 'center', minWidth: 80, paddingVertical: 12 },
  verifyBtnText:   { color: '#FFF', fontSize: 14, fontWeight: '800' },
  resultBox:       { flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 12, padding: 14, borderWidth: 1, marginBottom: 6 },
  resultTitle:     { fontSize: 14, fontWeight: '800' },
  resultSub:       { fontSize: 12, color: P.sub, marginTop: 2 },
  ...
  modalTitle:      { fontSize: 18, fontWeight: '900', color: P.text },
  fieldLabel:      { fontSize: 12, fontWeight: '700', color: P.sub, marginBottom: 6 },
  fieldInput:      { backgroundColor: P.bg, borderWidth: 1.5, borderColor: P.border, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, fontSize: 15, color: P.text, marginBottom: 8 },
  mResultOk:       { fontSize: 13, color: P.green,  fontWeight: '700', marginTop: 10 },
  mResultFail:     { fontSize: 13, color: P.danger, fontWeight: '700', marginTop: 10 },
  modalBtn:        { backgroundColor: P.navy, borderRadius: 14, paddingVertical: 15, alignItems: 'center', marginTop: 16 },
  modalBtnText:    { color: '#FFF', fontSize: 15, fontWeight: '800' },
});
