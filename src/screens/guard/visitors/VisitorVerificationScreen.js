...
import {
  View, Text, TouchableOpacity, StyleSheet, SafeAreaView, StatusBar,
  TextInput, ScrollView, KeyboardAvoidingView, Platform, Alert, Modal, FlatList, ActivityIndicator,
} from 'react-native';
import { useAuthStore } from '../../../store/AuthStore';
import { useSecurityStore } from '../../../store/securityStore';
import { useTheme } from '../../../hooks/useTheme';
  ...
  return (
    <View style={styles.modeToggle}>
      {[{ k: 'otp', label: '🔢 Enter OTP' }, { k: 'qr', label: '📱 Scan QR' }].map(m => (
        ...
        >
          <Text style={[styles.modeBtnText, mode === m.k && styles.modeBtnTextActive]}>
            {m.label}
          </Text>
        </TouchableOpacity>
      ))}
            ...
            >
              ...
              {showAvatars && (
                <View style={styles.avatarRow}>
                  {PHOTO_AVATARS.map(a => (
                    <TouchableOpacity
                      key={a}
                      onPress={() => { setPhoto(a); setShowAvatars(false); }}
                      style={[styles.avatarOpt, photo === a && styles.avatarOptActive]}
                    >
                      <Text style={{ fontSize: 26 }}>{a}</Text>
                    </TouchableOpacity>
                  ))}
              ...
              <View style={styles.chipRow}>
                {GATES.map(g => (
                  <TouchableOpacity
                    key={g}
                    style={[styles.chip, gate === g && styles.chipActive]}
                    onPress={() => setGate(g)}
                  >
                    <Text style={[styles.chipText, gate === g && { color: '#FFFFFF' }]}>{g}</Text>
                  </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                onPress={() => onConfirm({ photo, vehicleNote: vehicleNo || vehicleType, gate })}
              >
                <Text style={styles.confirmBtnText}>✅ CONFIRM ENTRY</Text>
              </TouchableOpacity>
            </ScrollView>
      ...
      <View style={styles.overlay}>
        <View style={[styles.modalBox, { maxHeight: '70%' }]}>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={{ textAlign: 'center', color: '#7A9E9E', padding: 20 }}>
...
function VisitorCard({ visitor, onCheckIn, onDeny }) {
  const S = {
    CREATED: '#F59E0B', APPROVED: '#0F766E',
    CHECKED_IN: '#1D4ED8', CHECKED_OUT: '#64748B', DENIED: '#DC2626',
  };
  const color = S[visitor.status] || '#64748B';
  return (
    <View style={styles.card}>
        ...
        {visitor.checkedInAt && (
          <View style={styles.detailRow}>
            ...
            <Text style={styles.detailValue}>
              {visitor.entryGate} • {visitor.verifiedByName || visitor.verifiedBy}
            </Text>
          </View>
        )}
        {visitor.photo && (
...
export default function VisitorVerificationScreen({ navigation }) {
  ...
  const checkInVisitor = useSecurityStore(s => s.checkInVisitor);
  const verifyGuestParkingOTP = useSecurityStore(s => s.verifyGuestParkingOTP);
  const denyVisitor = useSecurityStore(s => s.denyVisitor);
  const verifyVisitorOTP = useSecurityStore(s => s.verifyVisitorOTP);
  const verifyVisitorQR = useSecurityStore(s => s.verifyVisitorQR);
  const checkBlacklist = useSecurityStore(s => s.checkBlacklist);
  ...
  const [parkingOtpInput, setParkingOtpInput] = useState('');
  const [foundParking, setFoundParking] = useState(null);
  const [parkingVerifying, setParkingVerifying] = useState(false);
  const [parkingAllowing, setParkingAllowing] = useState(false);

  const guardName = user?.name || 'Guard';
  const guardId = user?.id || 'sec1';

  // ── OTP Verify: backend first, local fallback ─────────────────────────────
  // POST /api/visitor/verify-otp — SECURITY role, JWT required
  // Body: { otp } only — guard identity from JWT
  const handleOTPVerify = async () => {
    ...
    setVerifying(true);
    try {
      const { response, data } = await verifyVisitorOtpApi(token, otpInput.trim());
      if (response.ok && data) {
        const visitor = {
          id: String(data.id),
        ...
        const { ok: localOk, visitor } = verifyVisitorOTP(otpInput.trim());
        if (!localOk) {
          Alert.alert('Not Found', 'No approved visitor found with this OTP.');
          setVerifying(false);
          return;
        }
        const bl = checkBlacklist(visitor.name, visitor.phone);
  ...
  const handleQRScan = (qrData) => {
    ...
    const { ok, visitor } = verifyVisitorQR(qrData);
    if (!ok) {
      Alert.alert('Invalid QR', 'No approved visitor found.');
      return;
    }
    const bl = checkBlacklist(visitor.name, visitor.phone);
    if (bl) {
      Alert.alert('🚫 BLACKLISTED', `${visitor.name} is blacklisted.\nReason: ${bl.reason}`);
      return;
    }
    setFoundVisitor(visitor);
  };
  ...
  // ── Check-In: backend first, always update local store ────────────────────
  // POST /api/visitor/checkin/{passId} — SECURITY role, JWT required
  // Body: { gate } only — guard identity from JWT
  const handleConfirmCheckIn = async ({ photo, vehicleNote, gate }) => {
    setShowCheckIn(false);
    setCheckingIn(true);
    try {
      if (foundVisitor.isFromBackend && foundVisitor.backendId) {
        await checkInVisitorApi(token, foundVisitor.backendId, gate);
      }
    } catch (e) {
      console.warn('Backend check-in failed, updating local only');
    }
    // Always update local store
    checkInVisitor(foundVisitor.id, guardId, guardName, gate, photo, vehicleNote);
    setFoundVisitor(prev => ({
      ...prev,
      status: 'CHECKED_IN',
      entryGate: gate,
      verifiedByName: guardName,
      photo,
      guardVehicleNote: vehicleNote,
    }));
    setCheckingIn(false);
    Alert.alert(
      '✅ Entry Allowed',
      `${foundVisitor.name} has been checked in at ${gate}.\n\nResident will be notified automatically.`
    );
  };

  // ── Deny: backend first, always update local store ────────────────────────
  // POST /api/visitor/deny/{passId} — SECURITY role, JWT required
  const handleDeny = (visitor) => {
    Alert.alert('Deny Entry', `Deny ${visitor.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          if (visitor.isFromBackend && visitor.backendId) {
            try {
              await denyVisitorApi(token, visitor.backendId);
            } catch (e) {
              console.warn('Backend deny failed, updating local only');
            }
          }
          denyVisitor(visitor.id, guardId);
          setFoundVisitor(prev => ({ ...prev, status: 'DENIED' }));
          Alert.alert('Entry Denied', `${visitor.name} has been denied entry.`);
        },
      },
    ]);
  };
  ...
  // ── Parking OTP verify ────────────────────────────────────────────────────
  const handleParkingVerify = async () => {
    const otp = parkingOtpInput.trim();
    if (otp.length !== 6) {
      Alert.alert('Invalid', 'Enter a valid 6-digit parking OTP');
      return;
    }
    setParkingVerifying(true);
    await fetchGuestParking?.();
    const latestParking = useSecurityStore.getState().guestParking || [];
    const parking = latestParking.find(
      item =>
        String(item.parkingOtp || '').trim() === otp &&
        String(item.status || '').toUpperCase() === 'APPROVED'
    );
    const alreadyActive = latestParking.find(
      item =>
        String(item.parkingOtp || '').trim() === otp &&
        String(item.status || '').toUpperCase() === 'ACTIVE'
    );
    const exitedParking = latestParking.find(
      item =>
        String(item.parkingOtp || '').trim() === otp &&
        ['EXITED', 'EXPIRED'].includes(String(item.status || '').toUpperCase())
    );
    const rejectedParking = latestParking.find(
      item =>
        String(item.parkingOtp || '').trim() === otp &&
    ...
    if (alreadyActive) {
      setParkingVerifying(false);
      Alert.alert('Already Active', 'This parking pass is already checked in.');
      return;
    }
    if (exitedParking) {
      setParkingVerifying(false);
      Alert.alert('Closed Pass', 'This parking pass is already closed.');
      return;
    }
    if (rejectedParking) {
      setParkingVerifying(false);
      Alert.alert('Rejected', 'This guest parking request was rejected.');
      return;
    }
    if (!parking) {
      setParkingVerifying(false);
      Alert.alert('Not Found', 'No approved parking request found for this OTP.');
      return;
    }

    setFoundParking(parking);
    setParkingOtpInput('');
    setParkingVerifying(false);
  };

  const handleAllowParking = async () => {
    if (!foundParking?.parkingOtp) return;
    setParkingAllowing(true);
    const result = await verifyGuestParkingOTP(
      foundParking.parkingOtp,
      String(guardId),
      guardName,
      'Main Gate'
    );
    setParkingAllowing(false);
    if (!result?.ok) {
      Alert.alert(
        'Unable to allow entry',
        result?.reason || 'Could not activate this parking request.'
      );
      return;
    }
    const updatedParking =
      result.parking ||
      (useSecurityStore.getState().guestParking || []).find(item => item.id === foundParking.id);
    setFoundParking(
      updatedParking || { ...foundParking, status: 'ACTIVE', entryGate: 'Main Gate' }
    );
    Alert.alert(
      'Entry Allowed',
      `${foundParking.guestName} vehicle is checked in for slot ${foundParking.slotNumber}.`,
      [
        {
          text: 'Open Vehicle Logs',
          onPress: () =>
            navigation.navigate('VehicleEntry', {
              prefill: {
                parkingId: updatedParking?.id || foundParking.id,
                vehicleNumber: updatedParking?.vehicleNumber || foundParking.vehicleNumber || '',
                ownerName: updatedParking?.guestName || foundParking.guestName || '',
                flatNumber: updatedParking?.unit || foundParking.unit || '',
                vehicleType: updatedParking?.vehicleType || foundParking.vehicleType || 'Car',
                purpose: 'Visitor (Guest Parking)',
                parkingSlot: updatedParking?.slotNumber || foundParking.slotNumber || '',
              },
            }),
        },
  ...
  const recentLogs = entryLogs.filter(l => l.type === 'VISITOR').slice(0, 8);
  const parkingLogs = entryLogs.filter(l => l.type === 'PARKING').slice(0, 8);
  ...
  return (
    <SafeAreaView style={[styles.screen, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        <ModeToggle
          mode={mode}
          onChange={m => { setMode(m); setFoundVisitor(null); setOtpInput(''); }}
        />
          ...
          <View style={styles.card}>
            ...
            {checkingIn && (
              <View style={{
                ...
                backgroundColor: '#DBEAFE', padding: 10, borderRadius: 10,
              }}>
                <ActivityIndicator color="#1D4ED8" size="small" />
                <Text style={{ color: '#1D4ED8', fontWeight: '700' }}>Processing check-in...</Text>
              </View>
            )}
            <View style={styles.otpRow}>
              <TextInput
        ...
        <Text style={styles.sectionLabel}>GUEST PARKING OTP</Text>
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Verify Parking by OTP</Text>
          <Text style={styles.cardSub}>Enter the parking OTP shared by the resident</Text>
          <View style={styles.otpRow}>
            <TextInput
              style={styles.otpInput}
              placeholder="6-digit Parking OTP"
              placeholderTextColor="#94A3B8"
              value={parkingOtpInput}
              onChangeText={setParkingOtpInput}
              keyboardType="number-pad"
              maxLength={6}
            />
            <TouchableOpacity
              style={styles.verifyBtn}
              onPress={handleParkingVerify}
              disabled={parkingVerifying}
            >
              {parkingVerifying
                ? <ActivityIndicator color="#FFFFFF" size="small" />
                : <Text style={styles.verifyBtnText}>Verify</Text>
        ...
        {foundParking && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Parking Verification Result</Text>
            <View style={styles.detailsBox}>
              {[
                { l: 'Guest', v: foundParking.guestName },
                { l: 'Vehicle', v: `${foundParking.vehicleNumber || '—'} (${foundParking.vehicleType || 'Car'})` },
                { l: 'Slot', v: foundParking.slotNumber || '—' },
                { l: 'Unit', v: foundParking.unit || '—' },
                { l: 'Resident', v: foundParking.residentName || '—' },
                { l: 'Status', v: foundParking.status || 'APPROVED' },
              ].map(row => (
            ...
            {String(foundParking.status || '').toUpperCase() !== 'ACTIVE' ? (
              <TouchableOpacity
                style={styles.checkInBtn}
                onPress={handleAllowParking}
                disabled={parkingAllowing}
              >
                {parkingAllowing
                  ? <ActivityIndicator color="#FFFFFF" size="small" />
                  : <Text style={styles.checkInBtnText}>Allow Parking Entry</Text>
                }
              </TouchableOpacity>
            ) : (
              <View style={styles.successRow}>
                <Text style={styles.successText}>
                  Checked in at {foundParking.entryGate || 'Main Gate'}
                </Text>
              </View>
            )}
          </View>
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.cardSub}>
                    {log.unit} · {log.gate} · {log.guardName || log.guardId}
                  </Text>
                  {log.vehicleNote ? (
                    <Text style={[styles.cardSub, { color: theme.primary }]}>🚗 {log.vehicleNote}</Text>
                  ) : null}
                </View>
                <Text style={{ fontSize: 11, color: theme.textMuted }}>
                  {new Date(log.at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                </Text>
              </View>
            ))}
          </>
        ...
        {parkingLogs.length > 0 && (
          <>
            <Text style={styles.sectionLabel}>RECENT PARKING ENTRIES</Text>
            {parkingLogs.map(log => (
              <View
                key={log.id}
                style={[styles.card, { flexDirection: 'row', alignItems: 'center', gap: 10 }]}
              >
                <Text style={{ fontSize: 20 }}>{log.action === 'CHECK_IN' ? 'P' : 'E'}</Text>
                <View style={{ flex: 1 }}>
                  <Text style={styles.cardTitle}>{log.name}</Text>
                  <Text style={styles.cardSub}>
                    {log.unit} · {log.gate} · {log.guardName || log.guardId}
                  ...
                  {log.vehicleNote ? (
                    <Text style={[styles.cardSub, { color: theme.primary }]}>Vehicle {log.vehicleNote}</Text>
                  ) : null}
                </View>
                <Text style={{ fontSize: 11, color: theme.textMuted }}>
                  {new Date(log.at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
