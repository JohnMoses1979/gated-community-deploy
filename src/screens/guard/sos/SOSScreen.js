...
/**
 * SOSScreen.js — Guard SOS management
 *
 * Architecture:
 *   - All API calls via securityStore (acknowledgeSOS, respondSOS, resolveSOS, fetchSOSAlerts)
 *   - Token from AuthStore — never from request body
 *   - fetchSOSAlerts() called on focus + 10s polling
 *   - guardId/guardName NOT passed to store actions — backend reads from JWT
 *   - navigation prop scoped to screen only — never passed into card sub-components
 */

import React, { useCallback, useRef, useEffect, useState } from 'react';
import {
  View, Text, TouchableOpacity, StyleSheet, SafeAreaView, StatusBar,
  ScrollView, Alert, Animated, TextInput, Modal, RefreshControl,
} from 'react-native';
...
// ─── Pulse animation for TRIGGERED cards ─────────────────────────────────────
function PulseCard({ children, triggered }) {
  const pulseAnim = useRef(new Animated.Value(1)).current;
...
// ─── SOS Card ─────────────────────────────────────────────────────────────────
// navigation is NOT a prop here — avoids the broken navigation.goBack() bug
// that existed in the source file.
function SOSCard({ sos, onAcknowledge, onRespond, onResolve }) {
  const meta = STATUS_META[sos.status] || STATUS_META.TRIGGERED;
  const isTriggered = sos.status === 'TRIGGERED';
    ...
    <PulseCard triggered={isTriggered}>
      <View style={[styles.sosCard, { borderLeftColor: meta.color, borderLeftWidth: 4 }]}>
        ...
        <View style={styles.sosDetails}>
          ...
          <Text style={styles.sosTime}>🕒 Triggered: {fmt(sos.triggeredAt)}</Text>
          {sos.acknowledgedByName && (
            <Text style={styles.sosTime}>👀 Acknowledged by: {sos.acknowledgedByName}</Text>
          )}
          {sos.resolvedByName && (
        ...
        {/* Timeline */}
        {Array.isArray(sos.timeline) && sos.timeline.length > 0 && (
          <View style={styles.timeline}>
            {sos.timeline.map((t, i) => (
              <View key={i} style={styles.timelineRow}>
                <View style={[styles.timelineDot, { backgroundColor: i === 0 ? meta.color : '#CBD5E1' }]} />
                <View style={{ flex: 1 }}>
                  <Text style={styles.timelineAction}>{t.action}</Text>
                  <Text style={styles.timelineAt}>{fmt(t.at)}</Text>
                </View>
              </View>
            ))}
          </View>
        ...
        {sos.status === 'ACKNOWLEDGED' && (
          <View style={styles.actionRow}>
            <TouchableOpacity
              style={[styles.actionBtnHalf, { backgroundColor: '#1A7A7A' }]}
              onPress={() => onRespond(sos.id)}
            >
              <Text style={styles.actionBtnText}>🏃 Responding</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.actionBtnHalf, { backgroundColor: '#1A7A7A' }]}
              onPress={() => onResolve(sos)}
            >
              <Text style={styles.actionBtnText}>✅ Resolve</Text>
            </TouchableOpacity>
          </View>
        ...
        {sos.status === 'IN_PROGRESS' && (
          <TouchableOpacity
            style={[styles.actionBtn, { backgroundColor: '#1A7A7A' }]}
            onPress={() => onResolve(sos)}
          >
            <Text style={styles.actionBtnText}>✅ Mark Resolved</Text>
          </TouchableOpacity>
...
export default function SOSScreen({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      // Read fresh reference from store to avoid stale closure
    ...
    }, []) // empty deps — intentional, we read from store directly
  );
  ...
  // guardId/guardName NOT passed — backend reads from JWT
  const handleAcknowledge = useCallback(async (id) => {
    await acknowledgeSOS(id);
    Alert.alert('Acknowledged', 'You have acknowledged this SOS. Please respond immediately.');
  }, [acknowledgeSOS]);

  const handleRespond = useCallback(async (id) => {
    await respondSOS(id);
    Alert.alert('Status Updated', 'Marked as In Progress. Please resolve once situation is handled.');
  }, [respondSOS]);

  const handleResolve = useCallback((sos) => {
    setResolveTarget(sos);
  }, []);

  const confirmResolve = useCallback(async (note) => {
    await resolveSOS(resolveTarget.id, note);
    setResolveTarget(null);
    Alert.alert('✅ Resolved', 'SOS has been marked as resolved.');
  }, [resolveSOS, resolveTarget]);
