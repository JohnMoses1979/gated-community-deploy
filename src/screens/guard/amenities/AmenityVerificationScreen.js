 ...
 *
 * SECURITY FIX (merged):
 *  - guardId and guardName removed from /verify-otp request body
 *    (backend extracts guard identity from JWT via CurrentUser.get())
 *  - Authorization: Bearer <token> header added to all fetch calls
 *  - SecurityConfig enforces hasRole("SECURITY") for /verify-otp endpoints
 */
import React, { useState } from 'react';
import {
  View, Text, TouchableOpacity, StyleSheet, SafeAreaView,
...
const C = {
  ...
  danger: '#DC2626', bg: '#F0FAFA', card: '#FFF',
...
export default function AmenityVerificationScreen({ navigation }) {
  const theme = useTheme();
  const user = useAuthStore(s => s.user);
  // FIX: get token for Authorization header — guard identity is in JWT
  ...
  const handleVerify = async () => {
    ...
    try {
      const endpoint = mode === 'ev'
        ? apiUrl('/ev-bookings/verify-otp')
        : apiUrl('/amenity-bookings/verify-otp');

      // FIX: only send otp — guardId and guardName removed
      // Backend reads guard identity from JWT (CurrentUser.get())
      ...
      setVerified(data.ok);
      if (!data.ok) {
        Alert.alert('❌ Invalid', data.reason || 'OTP not found or already used');
      }
    } catch (e) {
      Alert.alert('Error', 'Could not connect to server. Check network.');
    }
  };
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.modeRow}>
          {[{ id: 'amenity', label: '🏊 Amenity' }, { id: 'ev', label: '⚡ EV Charging' }].map(m => (
            <TouchableOpacity
              key={m.id}
              style={[s.modeBtn, mode === m.id && s.modeBtnA]}
              onPress={() => { setMode(m.id); handleReset(); }}
            >
              <Text style={[s.modeBtnT, mode === m.id && s.modeBtnTA]}>{m.label}</Text>
            </TouchableOpacity>
          ))}
