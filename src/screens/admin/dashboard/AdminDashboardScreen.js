...
export default function AdminHomeScreen({ navigation }) {
  ...
  const seededMaintenanceReqs = useAppStore(s => s.maintenanceRequests);
  const maintenanceRequests = useMaintenanceStore(s => s.requests);
  const adminFetchMaintenance = useMaintenanceStore(s => s.adminFetchAll);
  ...
  useFocusEffect(
    useCallback(() => {
      fetchPendingUsers?.();
      adminFetchMaintenance?.(useAuthStore.getState().token);
      // Fetch SOS alerts fresh on every focus — admin sees all alerts
      useSecurityStore.getState().fetchSOSAlerts();

      // Poll every 15s so admin dashboard stays live
      const sosInterval = setInterval(() => {
        useSecurityStore.getState().fetchSOSAlerts();
      }, 15000);

      return () => clearInterval(sosInterval);
    }, [fetchPendingUsers, adminFetchMaintenance])
  );
  ...
  const maintenanceReqs = maintenanceRequests?.length ? maintenanceRequests : seededMaintenanceReqs;
  const openMaintenance = maintenanceReqs?.filter(r => !['closed', 'quote_rejected'].includes(r.status))?.length ?? 0;
  const urgentMaintenance = maintenanceReqs?.filter(r => ['high', 'urgent'].includes(String(r.priority || '').toLowerCase()))?.length ?? 0;
        ...
        <View style={s.body}>
          ...
          {activeSOS.length > 0 && (
            <SOSAlertBanner
              ...
              latest={latestSOS}
              onPress={() => navigation.navigate('SOSAdmin')}
            />
          )}
