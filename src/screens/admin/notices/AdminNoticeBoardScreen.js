 ...
 *
 * SECURITY FIX (merged):
 *  - apiPostNotice(data, token) — now passes JWT token instead of user?.id
 *    (backend derives admin identity from JWT; no ?adminId= param sent)
...
export default function AdminNoticeBoardScreen({ navigation }) {
  const theme = useTheme();
  const user = useAuthStore(s => s.user);
  // FIX: get token for Authorization header instead of user.id as adminId
  ...
  const handlePost = async () => {
    ...
    const data = {
      ...
      pinned,
      expiresAt: new Date(Date.now() + parseInt(expiryDays || 7) * 86400000).toISOString(),
      ...(isEvent ? { eventDate, eventTime, eventVenue, rsvpEnabled } : {}),
    };
    try {
      // FIX: pass token instead of user?.id
      // Backend reads adminId from JWT, no query param needed
      const saved = await apiPostNotice(data, token);
      addAnnouncement(saved, user?.id, user?.name || 'Admin');
      resetForm();
      setShowCompose(false);
      Alert.alert('✅ Posted!', 'Your announcement has been saved to the database.');
    } catch (err) {
      Alert.alert('❌ Failed', err.message || 'Could not save notice. Is the server running?');
    }
  };

  const handleDelete = (id, titleText) => {
    Alert.alert('Delete?', `Remove "${titleText}"?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => deleteAnnouncement(id) },
    ]);
  };
