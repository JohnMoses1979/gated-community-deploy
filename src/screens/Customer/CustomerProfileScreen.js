...
import { fetchCustomerVisits, fetchMyBookings } from "../../api/builderApi";
...
export default function CustomerProfileScreen({ navigation }) {
  ...
  const parseMoney = (value) => {
    ...
    if (str.includes("cr")) return base * 10000000;
    if (str.includes("l")) return base * 100000;
    if (str.includes("k")) return base * 1000;
    return base;
  };

  const getBookingTotalAmount = (booking) =>
    Number(booking.totalAmount || booking.bookingAmount || booking.unit?.priceValue || parseMoney(booking.price || booking.unit?.price));
  ...
  const normalizeStatus = (status, type = "visit") => {
    const value = String(status || "").toUpperCase();
    if (type === "visit") {
      return { REQUESTED: "Pending", CONFIRMED: "Approved", COMPLETED: "Completed", CANCELLED: "Rejected" }[value] || "Pending";
    }
    return { PENDING: "Pending Approval", APPROVED: "Approved", REJECTED: "Rejected", CANCELLED: "Rejected" }[value] || "Pending Approval";
  };

  const normalizeVisit = (visit) => ({
    ...visit,
    status: normalizeStatus(visit.status, "visit"),
    projectName: visit.projectName || visit.project?.name || "Project",
    unitNumber: visit.unitNumber || "Project Visit",
    unitType: visit.unitType || "Unit",
    visitDate: visit.scheduledDate
      ? new Date(visit.scheduledDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })
      : "",
    visitTime: visit.scheduledDate
      ? new Date(visit.scheduledDate).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", hour12: true })
      : "",
    builderMessage: visit.builderMessage || "",
  });

  const normalizeBooking = (booking) => ({
    ...booking,
    status: normalizeStatus(booking.status, "booking"),
    projectName: booking.projectName || booking.unit?.projectName || "Project",
    unitNo: booking.unitNo || booking.unitNumber || booking.flatNo || booking.unit?.flatNo || booking.unit?.unitNumber || "Unit",
    unitType: booking.unitType || booking.unit?.bhkType || booking.unit?.unitType || "Flat",
    phone: booking.phone || booking.customer?.phone || user?.phone || "",
    price: booking.price || booking.unit?.price || "",
  });

  const ensureCustomerToken = useCallback(async () => {
    const state = useAuthStore.getState();

    // Already have valid token
    if (isBackendToken(state.token)) return state.token;

    // No phone — cannot authenticate
    if (!state.user?.phone) {
      console.log('No phone available for customer auth');
      return null;
    }

    // Try to re-authenticate
    const result = await state.authenticateCustomerProfile(state.user);
    const refreshedToken = useAuthStore.getState().token;

    if (isBackendToken(refreshedToken)) return refreshedToken;

    console.log('Customer re-auth failed:', result?.message);
    return null;
  }, []);

  // AFTER: prefer existing token directly, only re-auth if missing
  const refreshCustomerActivity = useCallback(async () => {
    // Prefer existing token directly first
    let token = useAuthStore.getState().token;
    if (!isBackendToken(token)) {
      token = await ensureCustomerToken();
    }
    if (!token) {
      console.log('No valid token for refreshCustomerActivity');
      return;
    }
    try {
      const [visitsResult, bookingsResult] = await Promise.all([
        fetchCustomerVisits(token),
        fetchMyBookings(token),
      ]);
      if (visitsResult.response?.ok && Array.isArray(visitsResult.data)) {
        setVisitRequests(visitsResult.data.map(normalizeVisit));
      }
      if (bookingsResult.response?.ok && Array.isArray(bookingsResult.data)) {
        // ADD: fetch installments for each booking to get real paidAmount
        console.log('RAW BOOKING DATA:', JSON.stringify(bookingsResult.data[0]));
        const bookingsWithPayments = await Promise.all(
          bookingsResult.data.map(async (booking) => {
            try {
              const { fetchMyInstallments } = require('../../api/builderApi');
              const instResult = await fetchMyInstallments(token, booking.id);
              if (instResult.response?.ok && Array.isArray(instResult.data)) {
                const paid = instResult.data
                  .filter(i => i.status === 'PAID')
                  .reduce((sum, i) => sum + Number(i.amount || 0), 0);
                return { ...booking, paidAmount: paid };
              }
            } catch (_) { }
            return booking;
          })
        );
        setFlatBookingRequests(bookingsWithPayments.map(normalizeBooking));
      }
    } catch (error) {
      Alert.alert("Error", "Unable to load your visits and bookings.");
    }
  }, [ensureCustomerToken, user?.phone]);

  useFocusEffect(
    useCallback(() => {
      refreshCustomerActivity();
    }, [refreshCustomerActivity])
  );
  ...
  const payCustomAmount = (booking) => {
    const remaining = Math.max(getBookingTotalAmount(booking) - Number(booking.paidAmount || 0), 0);
    Alert.alert(
      "Payment",
      `Your remaining balance is ${formatMoney(remaining)}. Please contact your builder to record the payment. Your progress will update automatically.`
    );
  };
