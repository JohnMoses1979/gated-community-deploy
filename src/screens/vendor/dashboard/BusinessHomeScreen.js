...
export default function BusinessHomeScreen({ navigation }) {
  ...
  React.useEffect(() => {
    if (token) vendorFetchMyJobs(token);
  ...
  const myJobs = vendorJobs.length
    ? vendorJobs
    : requests.filter(r => r.assignedVendorId === vendorUser?.id);
  const newRequests  = myJobs.filter(r => r.status === 'assigned').length;
  const inProgress   = myJobs.filter(r => ['approved_to_start', 'work_in_progress'].includes(r.status)).length;
  const completed    = myJobs.filter(r => r.status === 'closed').length;
  const earnings     = myJobs.filter(r => r.status === 'closed').reduce((s, r) => s + (r.quote?.amount || r.finalAmount || 0), 0);
  ...
  const recentJobs = myJobs.slice(0, 5).map(r => ({
    ...
    status: ['approved_to_start', 'work_in_progress'].includes(r.status) ? 'In Progress' : r.status === 'assigned' ? 'New' : r.status === 'closed' ? 'Completed' : r.status,
    statusColor: ['approved_to_start', 'work_in_progress'].includes(r.status) ? Colors.amber : r.status === 'closed' ? Colors.green : Colors.teal,
    statusBg: ['approved_to_start', 'work_in_progress'].includes(r.status) ? Colors.amberLight : r.status === 'closed' ? Colors.greenLight : Colors.purpleLight,
