...
export default function VendorHomeScreen({ navigation }) {
  ...
  const seededRequests = useAppStore(s => s.maintenanceRequests);
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    vendorFetchMyJobs(token).finally(() => setRefreshing(false));
  ...
  const myJobs = vendorJobs.length
    ? vendorJobs
    : seededRequests.filter(r =>
        r.assignedVendorId === vendorUser?.id || r.invitedVendorIds?.includes(vendorUser?.id)
      );

  const pendingQuotes    = myJobs.filter(r => r.status === 'assigned').length;
  const activeJobs       = myJobs.filter(r => r.status === 'work_in_progress').length;
  const approvedJobs     = myJobs.filter(r => ['approved_to_start','work_in_progress'].includes(r.status)).length;
  ...
  const recentActivity = [
    ...myJobs.filter(r => r.status === 'assigned').slice(0, 2).map(r => ({
      id: 'q-' + r.id, emoji: '📋',
