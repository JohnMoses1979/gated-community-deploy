...
/**
 * RequestListScreen.js — Vendor
 *
 * SECURITY FIX (merged):
 *  - appStore replaced with maintenanceStore (vendorFetchMyJobs)
 *  - Token from useAuthStore(s => s.token)
 *  - vendorId never sent — backend reads from JWT
 *  - All data from backend via vendorJobs state
 */
...
const TABS = [
  { key: 'quote_requested',   label: '📤 Quote Req' },
  { key: 'quoted',            label: '💬 Quoted' },
  { key: 'approved_to_start', label: '🚀 Active' },
  { key: 'completed',         label: '✅ Done' },
];
...
const STATUS_BADGE = {
  ...
  work_in_progress:             { label: '🔧 Working',    color: P.amber,   bg: P.amberBg },
  work_completed:               { label: '✅ Done',        color: P.green,   bg: P.greenBg },
  resident_work_approved:       { label: '✅ Res Approved',color: P.green,   bg: P.greenBg },
  payment_requested_to_admin:   { label: '💳 Pmt Req',    color: '#EA580C', bg: '#FFEDD5' },
  payment_requested_to_resident:{ label: '⏳ Pmt Pending',color: '#EA580C', bg: '#FFEDD5' },
  payment_received:             { label: '💵 Pmt Rcvd',   color: '#0891B2', bg: '#E0F7FA' },
  paid_to_vendor:               { label: '💰 Paid',        color: P.green,   bg: P.greenBg },
};

const CATEGORY_ICONS = {
  Plumbing: '🚿', Electrical: '⚡', Painting: '🎨', Carpentry: '🪵',
  HVAC: '❄️', Appliances: '🫙', Structural: '🏗️', 'Pest Control': '🐛',
  Landscaping: '🌱', Other: '🔧',
};

function Badge({ status }) {
  const b = STATUS_BADGE[status] || { label: status?.replace(/_/g, ' ') || '—', color: P.textMuted, bg: '#F5F5F5' };
  return (
    <View style={{ paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, backgroundColor: b.bg }}>
      <Text style={{ fontSize: 10, fontWeight: '800', color: b.color }}>{b.label}</Text>
    </View>
...
function getTabStatuses(key) {
  switch (key) {
    case 'quote_requested':   return ['assigned'];
    case 'quoted':            return ['quoted'];
    case 'approved_to_start': return ['approved_to_start', 'work_in_progress'];
    case 'completed':         return ['work_completed', 'resident_work_approved', 'payment_requested_to_admin', 'payment_requested_to_resident', 'payment_received', 'closed'];
    default:                  return [];
  }
}
...
export default function RequestListScreen({ navigation }) {
  ...
  const tabCount = (key) =>
    vendorJobs.filter(r => getTabStatuses(key).includes(r.status)).length;

  const filteredJobs = vendorJobs.filter(r =>
    getTabStatuses(activeTab).includes(r.status)
  );
  ...
  const renderItem = ({ item }) => (
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={s.sub}>{item.category} · Unit {item.unit} · {item.residentName}</Text>
        <Text style={s.date}>
          {new Date(item.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })} · {item.id}
        </Text>
        ...
        {item.status === 'quoted' && (
          <Text style={[s.hint, { color: P.amber }]}>
            💬 Quote ₹{item.quote?.amount?.toLocaleString('en-IN')} sent — awaiting admin review
          </Text>
        )}
        {item.status === 'quote_sent_to_resident' && (
          <Text style={[s.hint, { color: '#0D9488' }]}>📨 Quote forwarded to resident — awaiting decision</Text>
        )}
        {item.status === 'quote_accepted' && (
          <Text style={[s.hint, { color: P.green }]}>✅ Accepted — admin must approve work start</Text>
        )}
        {item.status === 'quote_rejected' && (
          <Text style={[s.hint, { color: P.red }]}>❌ Quote rejected — submit a revised quote</Text>
        )}
        {item.status === 'approved_to_start' && (
          <Text style={[s.hint, { color: P.purple, fontWeight: '800' }]}>🚀 Approved! Proceed to gate with your OTP</Text>
        )}
        {item.status === 'work_in_progress' && (
          <Text style={[s.hint, { color: P.amber }]}>🔧 Work started — mark completed once done</Text>
        )}
        {item.status === 'work_completed' && (
          <Text style={[s.hint, { color: '#0D9488' }]}>🏁 Waiting for resident to approve work</Text>
        )}
        {item.status === 'resident_work_approved' && (
          <Text style={[s.hint, { color: P.green, fontWeight: '800' }]}>✅ Resident approved — request payment now</Text>
        )}
        ...
        {item.status === 'work_in_progress' && (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('ActiveWork', { jobId: item.id })}
          >
            <Text style={s.actionBtnText}>📝 Complete Work</Text>
          </TouchableOpacity>
        )}
        {item.status === 'resident_work_approved' && (
          <TouchableOpacity
            style={[s.actionBtn, { backgroundColor: P.green }]}
            onPress={() => navigation.navigate('WorkCompleted', { jobId: item.id })}
          >
            <Text style={s.actionBtnText}>💰 Request Payment</Text>
          </TouchableOpacity>
        )}
      </View>
    </TouchableOpacity>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 }}>
          <View>
            ...
            <Text style={s.headerSub}>
              {vendorJobs.length} total · {tabCount('quote_requested')} need quote
            </Text>
          </View>
          <TouchableOpacity style={s.bellBtn} onPress={() => navigation.navigate('VendorNotifications')}>
            <Text style={{ fontSize: 18 }}>🔔</Text>
...
const s = StyleSheet.create({
  ...
  header:        { backgroundColor: P.teal, paddingTop: 40, paddingBottom: 16, paddingHorizontal: 20 },
  backBtnCircle: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' },
  backBtnArrow:  { fontSize: 26, color: '#FFF', fontWeight: '300', lineHeight: 30, marginTop: -2 },
  headerTitle:   { fontSize: 20, fontWeight: '800', color: '#FFF' },
  headerSub:     { fontSize: 12, color: 'rgba(255,255,255,0.72)', marginTop: 1 },
  bellBtn:       { width: 38, height: 38, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' },
  tabBar:        { flexDirection: 'row', backgroundColor: P.surface, borderBottomWidth: 1, borderBottomColor: P.border },
  ...
  quoteChip:     { marginTop: 6, alignSelf: 'flex-start', backgroundColor: P.greenBg, borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: '#BBF7D0' },
  quoteChipText: { fontSize: 11, fontWeight: '700', color: P.green },
  actionBtn:     { marginTop: 8, alignSelf: 'flex-start', backgroundColor: '#0D9488', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20 },
  actionBtnText: { fontSize: 12, color: '#FFF', fontWeight: '700' },
  empty:         { alignItems: 'center', paddingTop: 60, gap: 8, paddingHorizontal: 32 },
  emptyTitle:    { fontSize: 17, fontWeight: '800', color: P.text },
  emptySub:      { fontSize: 14, color: P.textMuted, textAlign: 'center', lineHeight: 20 },
