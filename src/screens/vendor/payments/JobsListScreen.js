...
import {
  ...
  SafeAreaView, StatusBar, Modal, Alert, FlatList, ActivityIndicator, RefreshControl,
} from 'react-native';
...
function JobDetailModal({ job, onClose }) {
  ...
  const vendorRequestPaymentAction = useMaintenanceStore(s => s.vendorRequestPayment);
  const vendorRequestPayment = (id) => vendorRequestPaymentAction(token, id);
  const liveJob = vendorJobs.find(r => String(r.id) === String(job?.id)) || job;
    ...
    <Modal visible={!!job} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={{ flex: 1, backgroundColor: P.bg }}>
        ...
        <ScrollView contentContainerStyle={md.body} showsVerticalScrollIndicator={false}>
            ...
            <View style={md.card}>
              ...
              {pendingApproval ? (
                ...
                <TouchableOpacity
                  style={md.stageBtn}
                  onPress={() => {
                    vendorMarkWorkComplete(token, liveJob.id);
          ...
          {liveJob.status === 'resident_work_approved' && (
            <View style={md.card}>
          ...
          {/* payment states */}
          {['payment_requested_to_admin', 'payment_requested_to_resident', 'payment_received'].includes(liveJob.status) && (
            <View style={[md.infoCard, { borderColor: P.green }]}>
...
export default function JobsListScreen({ navigation }) {
  const token = useAuthStore(s => s.token);
  const jobs = useMaintenanceStore(s => s.vendorJobs);
  ...
  const tabStatuses = (key) => {
    if (key === 'inprogress') return ['approved_to_start', 'work_in_progress'];
    if (key === 'approved') return ['approved_to_start', 'work_in_progress'];
    if (key === 'completed') return ['work_completed', 'resident_work_approved', 'payment_requested_to_admin', 'payment_requested_to_resident', 'payment_received', 'closed'];
    return [];
  };

  const filtered = jobs.filter(j => {
    if (activeTab === 'all') return true;
    return tabStatuses(activeTab).includes(j.status);
  ...
  const tabsWithCounts = JOB_TABS.map(t => ({
    ...
    count: jobs.filter(j => {
      ...
      return tabStatuses(t.key).includes(j.status);
    }).length,
  }));

  const totalEarnings = jobs.filter(j => j.status === 'closed').reduce((s, j) => s + (j.quote?.amount || 0), 0);
      ...
      <View style={sc.header}>
        ...
        <View style={sc.statsRow}>
          {[
            ...
            { v: jobs.filter(j => j.status === 'closed').length, l: 'Completed', c: '#6EE7B7' },
