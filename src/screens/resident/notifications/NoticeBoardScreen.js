...
/**
 * NoticeBoardScreen.js — Module 12: Communication & Notifications (Resident)
 *
 * SECURITY FIX (merged):
 *  - apiGetNotices(role, token) — token now passed as second arg
 *  - apiRsvp(noticeId, token) — token now passed as second arg
 *  - Unread badge tracked per-announcement in local state
 *  - targetRole filtering: only show notices matching resident role or 'all'
 */
...
import { useAuthStore } from '../../../store/AuthStore';
import { apiGetNotices, apiRsvp } from '../../../api/noticeApi';
...
const TYPE_CONFIG = {
  notice: { emoji: '📋', color: '#0277BD', label: 'Notice' },
  event: { emoji: '🎉', color: '#1A7A7A', label: 'Event' },
  payment_reminder: { emoji: '💰', color: '#E8A020', label: 'Payment' },
  emergency: { emoji: '🚨', color: '#C62828', label: 'Emergency' },
};
...
function NoticeCard({ item, isRead, onPress, onRsvp, rsvping }) {
  ...
  return (
    <TouchableOpacity
      style={[
        st.card,
        item.pinned && { borderColor: cfg.color, borderWidth: 2 },
    ...
    >
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <View style={[st.typeTag, { backgroundColor: cfg.color + '20' }]}>
            <Text style={[st.typeText, { color: cfg.color }]}>{cfg.emoji} {cfg.label}</Text>
          </View>
          {item.pinned && (
            <View style={[st.typeTag, { backgroundColor: '#FEF3C7' }]}>
              <Text style={[st.typeText, { color: '#92400E' }]}>📌 Pinned</Text>
            </View>
          )}
        </View>
        {!isRead && (
      ...
      <Text style={[st.cardTitle, !isRead && { fontWeight: '900' }]}>{item.title}</Text>
      <Text style={st.cardBody} numberOfLines={3}>{item.body}</Text>

      {/* Event details */}
      {isEvent && item.eventDate && (
        <View style={[st.eventBox]}>
          <Text style={st.eventText}>📅 {item.eventDate}  ⏰ {item.eventTime || 'TBD'}</Text>
          {item.eventVenue && (
            <Text style={st.eventText}>📍 {item.eventVenue}</Text>
          )}
        </View>
      )}
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
        <Text style={st.meta}>{timeAgo(item.postedAt)}</Text>

        {isEvent && item.rsvpEnabled && !isExpired && (
          <TouchableOpacity
            style={[st.rsvpBtn, rsvping && { opacity: 0.6 }]}
            onPress={onRsvp}
            disabled={rsvping}
          >
            {rsvping
              ? <ActivityIndicator size="small" color="#FFF" />
              : <Text style={st.rsvpBtnT}>
                🎟️ RSVP {item.rsvpCount > 0 ? `(${item.rsvpCount})` : ''}
              </Text>
            }
          </TouchableOpacity>
        )}
        ...
        {isExpired && (
          <Text style={{ fontSize: 11, color: C.danger, fontWeight: '700' }}>⛔ Expired</Text>
        )}
      </View>
    </TouchableOpacity>
  );
...
export default function NoticeBoardScreen({ navigation }) {
  ...
  const [filter, setFilter] = useState('all'); // 'all' | 'notice' | 'event' | 'emergency'

  // FIX: token passed as second argument to apiGetNotices
  const fetchNotices = useCallback(async (isRefresh = false) => {
    if (isRefresh) setRefreshing(true);
    else setLoading(true);
    try {
      const data = await apiGetNotices(userRole, token);
      setNotices(Array.isArray(data) ? data : []);
    } catch (e) {
      Alert.alert('Error', 'Could not load notices. Check your connection.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  // FIX: token passed as second argument to apiRsvp
  const handleRsvp = async (notice) => {
    setRsvpingId(notice.id);
    try {
      const updated = await apiRsvp(notice.id, token);
      setNotices(prev =>
        prev.map(n => n.id === notice.id ? { ...n, rsvpCount: updated.rsvpCount } : n)
      );
      Alert.alert('🎟️ RSVP Confirmed!', `You're registered for "${notice.title}".`);
    } catch (e) {
      Alert.alert('RSVP Failed', 'Could not register. Please try again.');
    } finally {
      setRsvpingId(null);
    }
  };
    ...
    <SafeAreaView style={st.screen}>
      ...
      <View style={st.hdr}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View>
            <Text style={st.hdrT}>Notice Board</Text>
          ...
          {unreadCount > 0 && (
            <View style={st.badge}>
              <Text style={st.badgeT}>{unreadCount}</Text>
            </View>
          )}
        </View>
      </View>
      ...
      <View style={st.filterRow}>
        {FILTERS.map(f => (
          <TouchableOpacity
            ...
            onPress={() => setFilter(f.key)}
          >
            <Text style={[st.filterBtnT, filter === f.key && st.filterBtnTA]}>
              {f.label}
            </Text>
          </TouchableOpacity>
        ))}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={st.empty}>
              ...
              <Text style={st.emptyT}>No notices found</Text>
...
const st = StyleSheet.create({
  ...
  badge: { backgroundColor: C.danger, borderRadius: 12, minWidth: 24, height: 24, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 6 },
  badgeT: { color: '#FFF', fontSize: 12, fontWeight: '900' },
  ...
  filterBtnT: { fontSize: 11, fontWeight: '600', color: C.muted },
  ...
  cardBody: { fontSize: 13, color: C.muted, lineHeight: 19 },
  eventBox: { backgroundColor: '#E8F5F5', borderRadius: 10, padding: 10, marginTop: 10, gap: 4 },
  eventText: { fontSize: 12, color: C.primary, fontWeight: '700' },
  meta: { fontSize: 11, color: C.muted },
  rsvpBtn: { backgroundColor: C.primary, paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20 },
  rsvpBtnT: { color: '#FFF', fontSize: 12, fontWeight: '800' },
