...
/**
 * SOSTrackingScreen.js — Resident SOS management
 *
 * Architecture:
 *   - All API calls via securityStore (triggerSOS, fetchSOSAlerts, resolveSOS)
 *   - Token from AuthStore — never from body
 *   - fetchSOSAlerts() called on focus + 10s polling
 *   - Cancel = resolve with cancellation note (calls resolveSOS in store)
 *   - useFocusEffect for lifecycle, not useEffect, to avoid stale interval bugs
 */
...
function TimelineRow({ item, isLast }) {
  const isResolved = item.action?.includes('RESOLVED');
  const isFirst = item.action?.includes('TRIGGERED');
  const dotColor = isResolved ? T.success : isFirst ? T.danger : '#1976D2';
  return (
    <View style={tl.row}>
...
function SOSCard({ alert, onCancel }) {
  ...
  return (
    ...
    >
      ...
      {isActive && (
        <View style={styles.activePulseBar}>
          <Text style={styles.activePulseText}>🔴  ACTIVE — Guards & Admin are notified</Text>
        </View>
      )}

      {!expanded && (
        <Text style={styles.expandHint}>Tap to see details & timeline ›</Text>
      )}
        ...
        <View style={styles.expandedBody}>
          ...
          {Array.isArray(alert.timeline) && alert.timeline.length > 0 && (
            <View style={{ marginTop: 12 }}>
              <Text style={styles.sectionLabel}>RESPONSE TIMELINE</Text>
              {alert.timeline.map((t, i) => (
                <TimelineRow key={i} item={t} isLast={i === alert.timeline.length - 1} />
              ))}
            </View>
          )}

          {isCancellable && (
            <TouchableOpacity style={styles.cancelAlertBtn} onPress={() => onCancel(alert)}>
              <Text style={styles.cancelAlertText}>✕  Cancel This SOS Alert</Text>
            </TouchableOpacity>
          )}
        </View>
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: '🚨 SEND SOS NOW',
          style: 'destructive',
          onPress: () => { onSend(sosType, sosDesc.trim()); reset(); onClose(); },
        },
      ]
    );
  };
...
const modal = StyleSheet.create({
  ...
  closeX: { fontSize: 13, fontWeight: '700', color: '#64748B' },
  warningBox: { backgroundColor: '#FFF3F3', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: '#FFCDD2', marginBottom: 18 },
  warningText: { fontSize: 12, fontWeight: '700', color: '#8B0000', textAlign: 'center', lineHeight: 18 },
  fieldLabel: { fontSize: 11, fontWeight: '700', color: T.textSub, letterSpacing: 0.6, marginBottom: 8 },
  typeRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 },
  typeChip: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#FFF0F0', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: T.border },
  typeChipActive: { backgroundColor: T.primary, borderColor: T.primary },
  typeChipText: { fontSize: 12, fontWeight: '700', color: T.textSub },
  input: { backgroundColor: T.inputBg, borderWidth: 1.5, borderColor: T.inputBorder, borderRadius: 12, padding: 12, fontSize: 14, color: T.text, minHeight: 80, marginBottom: 18 },
  sendBtn: { backgroundColor: T.primary, paddingVertical: 15, borderRadius: 14, alignItems: 'center' },
  sendBtnText: { color: T.textInverse, fontSize: 16, fontWeight: '900', letterSpacing: 1 },
  cancelBtn: { paddingVertical: 14, alignItems: 'center' },
  cancelBtnText: { fontSize: 14, fontWeight: '600', color: T.textMuted },
});
...
export default function SOSTrackingScreen({ navigation }) {
  ...
  const sosAlerts = useSecurityStore(s => s.sosAlerts);
  const triggerSOS = useSecurityStore(s => s.triggerSOS);
  const fetchSOSAlerts = useSecurityStore(s => s.fetchSOSAlerts);
  ...
  // Focus + polling — cleaned up on blur to avoid duplicate intervals
  useFocusEffect(
    useCallback(() => {
      const fetch = useSecurityStore.getState().fetchSOSAlerts;
      fetch();
      const interval = setInterval(() => {
        useSecurityStore.getState().fetchSOSAlerts();
      }, 10000);
  ...
  // Resident sees only their own alerts (filtered by residentId from store)
  const myId = user?.id != null ? String(user.id) : null;
  const myAlerts = myId
    ? sosAlerts
      .filter(a => String(a?.residentId) === myId)
      .sort((a, b) => new Date(b.triggeredAt) - new Date(a.triggeredAt))
    : [];

  const filteredAlerts =
    filter === 'active' ? myAlerts.filter(a => a.status !== 'RESOLVED') :
      filter === 'RESOLVED' ? myAlerts.filter(a => a.status === 'RESOLVED') :
        myAlerts;

  const activeCount = myAlerts.filter(a => a.status !== 'RESOLVED').length;
  const resolvedCount = myAlerts.filter(a => a.status === 'RESOLVED').length;

  const handleSend = useCallback(async (type, description) => {
    const created = await triggerSOS(type, description, user?.unit || '');
    if (created) {
      setFilter('active');
      Alert.alert(
        '✅ SOS Sent!',
      ...
      Alert.alert('SOS Failed', 'Unable to send SOS right now. Please try again.', [{ text: 'OK' }]);
    }
  }, [triggerSOS, user]);

  // Cancel = resolve with a cancellation note (resident can only cancel TRIGGERED)
  const handleCancel = useCallback((alert) => {
    Alert.alert(
      'Cancel SOS Alert',
      `Cancel this ${alert.type} emergency alert?\n\nOnly cancel if this was a mistake.`,
      [
        { text: 'Keep Alert', style: 'cancel' },
        {
          text: 'Cancel Alert',
          style: 'destructive',
          onPress: async () => {
            await resolveSOS(alert.id, 'Alert cancelled by resident — false alarm.');
            // Force a fresh fetch so the resolved alert reflects immediately
            await fetchSOSAlerts();
            Alert.alert('Alert Cancelled', 'Your SOS alert has been cancelled.');
          },
        },
      ]
    );
  }, [resolveSOS, fetchSOSAlerts]);
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      >
        <View style={styles.infoBanner}>
          <Text style={styles.infoText}>
            Your SOS alerts are visible to ALL guards and admin in real-time. Tap{' '}
            <Text style={{ fontWeight: '800' }}>+ SOS</Text> to send an emergency alert instantly.
          </Text>
        </View>
        ...
        {myAlerts.length > 0 && (
          <View style={styles.statsRow}>
            <TouchableOpacity
              ...
              onPress={() => setFilter(filter === 'active' ? 'all' : 'active')}
            >
              <Text style={[styles.statNum, { color: activeCount > 0 ? T.danger : T.textMuted }]}>{activeCount}</Text>
              <Text style={[styles.statLabel, { color: activeCount > 0 ? T.danger : T.textMuted }]}>Active</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.statPill, { backgroundColor: T.successBg }]}
              onPress={() => setFilter(filter === 'RESOLVED' ? 'all' : 'RESOLVED')}
        ...
        {filteredAlerts.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            {filter !== 'all' && (
              <TouchableOpacity style={styles.showAllBtn} onPress={() => setFilter('all')}>
                <Text style={styles.showAllBtnText}>Show All Alerts</Text>
              </TouchableOpacity>
            )}
          </View>
      ...
      <SendSOSModal visible={showModal} onClose={() => setShowModal(false)} onSend={handleSend} />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: T.header, paddingTop: 44, paddingHorizontal: 20, paddingBottom: 14 },
  ...
  filterRow: { flexDirection: 'row', paddingBottom: 2, gap: 8 },
  ...
  infoBanner: { backgroundColor: T.infoBg, borderRadius: 12, padding: 14, marginBottom: 14, borderWidth: 1, borderColor: T.infoBorder },
  infoLabel: { fontSize: 10, fontWeight: '800', color: T.primary, letterSpacing: 0.5, marginBottom: 4 },
  infoText: { fontSize: 13, color: T.textSub, lineHeight: 20 },
  statsRow: { flexDirection: 'row', gap: 10, marginBottom: 14 },
  statPill: { flex: 1, borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
  statNum: { fontSize: 22, fontWeight: '900' },
  statLabel: { fontSize: 11, fontWeight: '700', marginTop: 2 },
  sectionLabel: { fontSize: 11, fontWeight: '800', color: T.textMuted, letterSpacing: 1, marginBottom: 10 },
  card: { backgroundColor: T.surface, borderRadius: 16, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: T.border, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  cardActive: { borderWidth: 2, borderColor: T.primary },
  cardHeader: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 8 },
  avatar: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: T.border },
  cardTitle: { fontSize: 15, fontWeight: '800', color: T.text },
  cardSub: { fontSize: 12, color: T.textMuted, marginTop: 3 },
  statusBanner: { marginBottom: 8, paddingVertical: 6, paddingHorizontal: 12, borderRadius: 8, borderWidth: 1, alignSelf: 'flex-start' },
  statusBannerText: { fontSize: 12, fontWeight: '700' },
  activePulseBar: { backgroundColor: T.dangerBg, borderRadius: 8, paddingVertical: 6, paddingHorizontal: 12, marginBottom: 8, alignItems: 'center' },
  activePulseText: { fontSize: 12, fontWeight: '800', color: T.danger, letterSpacing: 0.3 },
  expandHint: { fontSize: 11, color: T.textMuted, fontWeight: '600', textAlign: 'right', marginTop: 2 },
  expandedBody: { marginTop: 6 },
  dividerLine: { height: 1, backgroundColor: T.divider, marginBottom: 10 },
  responderBox: { backgroundColor: T.infoBg, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: T.infoBorder, marginBottom: 10 },
  responderTitle: { fontSize: 12, fontWeight: '800', color: T.primary, marginBottom: 8 },
  infoRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 5, borderBottomWidth: 1, borderBottomColor: T.divider },
  infoValue: { fontSize: 13, fontWeight: '700', color: T.text, maxWidth: '60%', textAlign: 'right' },
  cancelAlertBtn: { marginTop: 12, paddingVertical: 11, borderRadius: 12, borderWidth: 1.5, borderColor: T.border, alignItems: 'center', backgroundColor: T.infoBg },
  cancelAlertText: { fontSize: 13, fontWeight: '700', color: T.danger },
  emptyState: { alignItems: 'center', paddingVertical: 40, paddingHorizontal: 24 },
  emptyTitle: { fontSize: 17, fontWeight: '800', color: T.text, marginBottom: 8 },
  emptyDesc: { fontSize: 13, color: T.textMuted, textAlign: 'center', lineHeight: 20, marginBottom: 16 },
  showAllBtn: { backgroundColor: T.primary, paddingHorizontal: 24, paddingVertical: 10, borderRadius: 20 },
  showAllBtnText: { color: '#FFFFFF', fontWeight: '700', fontSize: 13 },
});