...
import {
  View, Text, TouchableOpacity, StyleSheet, SafeAreaView, StatusBar,
  ScrollView, Alert, Linking, Clipboard, ActivityIndicator,
} from 'react-native';
...
import { useTheme } from '../../../hooks/useTheme';
import { getMyVisitorPasses } from '../../../api/visitorApi';

// ─── Share Helper ─────────────────────────────────────────────────────────────
const shareVisitorPass = async (visitor) => {
  const msg =
    `🏠 *Visitor Pass*\n\n` +
    `👤 Name: ${visitor.name}\n` +
    `🔑 OTP: *${visitor.otp}*\n` +
...
const copyOTP = (visitor) => {
  const msg = `Visitor OTP for ${visitor.name}: ${visitor.otp}. Show at gate.`;
  try {
    Clipboard.setString(msg);
  } catch (e) {
    console.warn('Clipboard error:', e);
  }
  Alert.alert('✅ Copied!', `OTP ${visitor.otp} copied to clipboard.`);
};

// ─── Status meta ──────────────────────────────────────────────────────────────
const STATUS_META = {
  CREATED: { color: '#D97706', bg: '#FEF3C7', label: '⏳ OTP Generated — Share with visitor' },
  APPROVED: { color: '#0D6E6E', bg: '#CCFBF1', label: '✅ Approved — Visitor can enter' },
  CHECKED_IN: { color: '#1D4ED8', bg: '#DBEAFE', label: '🚶 Checked In — Visitor is inside' },
  CHECKED_OUT: { color: '#64748B', bg: '#F1F5F9', label: '🚪 Checked Out — Visitor has left' },
  DENIED: { color: '#C62828', bg: '#FEE2E2', label: '🚫 Entry Denied by Guard' },
};
...
export default function VisitorListScreen({ navigation }) {
  ...
  // ── Fetch from backend every time screen is focused ───────────────────────
  // Uses GET /api/visitor/my — JWT identifies the resident, no residentId in path
  useFocusEffect(
    useCallback(() => {
      const fetchFromBackend = async () => {
        ...
        setLoading(true);
        try {
          const { response, data } = await getMyVisitorPasses(token);
          if (response.ok && Array.isArray(data)) {
            const converted = data.map(d => ({
              ...
              status: d.status,
              hostUnit: d.hostUnit,
              hostResidentName: d.hostResidentName,
              entryGate: d.entryGate,
              verifiedByName: d.verifiedByGuardName,
              checkedInAt: d.checkedInAt,
              checkedOutAt: d.checkedOutAt,
              createdAt: d.createdAt,
              isFromBackend: true,
            }));
            setBackendVisitors(converted);
          }
        } catch (e) {
  ...
  // ── Merge: backend takes priority, fallback to local store ────────────────
  const localMyVisitors = visitors
    .filter(v => v.hostResidentId === myId)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

  const allVisitors = backendVisitors.length > 0 ? backendVisitors : localMyVisitors;
  const filtered = filter === 'all'
    ? allVisitors
    : allVisitors.filter(v => v.status === filter);
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 }}>
              ...
              <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)' }}>
                {loading ? 'Syncing...' : `${allVisitors.length} total visitors`}
              </Text>
            </View>
          </View>
          <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('AddVisitor')}>
          ...
          filtered.map(v => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  globalStyles.card,
                  { borderLeftWidth: 4, borderLeftColor: meta.color, padding: 12, marginBottom: 8 },
                ]}
                onPress={() => navigation.navigate('VisitorQRCode', { visitorId: v.id })}
                activeOpacity={0.85}
              >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <View style={{ flexDirection: 'row', flex: 1 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={globalStyles.cardSub}>{v.purpose}</Text>
                      {v.phone ? (
                        <Text style={globalStyles.cardSub}>📱 {v.phone}</Text>
                      ) : null}
                      {(v.guardVehicleNote || v.vehicleNumber) ? (
                        <Text style={globalStyles.cardSub}>🚗 {v.guardVehicleNote || v.vehicleNumber}</Text>
                      ) : null}
                    </View>
                  </View>
                  <View style={[styles.statusBadge, { backgroundColor: meta.bg }]}>
                ...
                {v.status === 'CHECKED_IN' && (
                  <View style={{ backgroundColor: '#DBEAFE', borderRadius: 8, padding: 8, marginTop: 8 }}>
                    <Text style={{ color: '#1D4ED8', fontWeight: '800', fontSize: 13 }}>✅ Checked In</Text>
                ...
                {v.status !== 'CHECKED_IN' && v.status !== 'CHECKED_OUT' && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 8 }}>
                    <View style={{ backgroundColor: '#E8F5F5', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5 }}>
                      <Text style={{ fontWeight: '900', letterSpacing: 3, fontFamily: 'monospace', color: '#0D6E6E', fontSize: 16 }}>
                        {v.otp}
                      </Text>
                    </View>
                    <TouchableOpacity
                      onPress={() => shareVisitorPass(v)}
                      style={{ backgroundColor: '#25D366', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5 }}
                    >
                      <Text style={{ color: '#fff', fontWeight: '700', fontSize: 12 }}>💬 Share</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      onPress={() => copyOTP(v)}
                      style={{ backgroundColor: '#6366F1', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5 }}
                    >
                      <Text style={{ color: '#fff', fontWeight: '700', fontSize: 12 }}>📋 Copy</Text>
                    </TouchableOpacity>
                  </View>
                )}
              </TouchableOpacity>
