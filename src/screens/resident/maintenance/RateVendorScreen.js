 ...
 *
 * SECURITY FIX (merged):
 *  - appStore.submitVendorRating replaced with maintenanceStore.submitRating
 *  - Token from useAuthStore(s => s.token)
 *  - Ionicons replaced with emoji to remove expo-vector-icons dependency risk
 */
import React, { useState } from 'react';
import {
  View, Text, TouchableOpacity, StyleSheet,
...
export default function RateVendorScreen({ navigation, route }) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    const result = await submitRating(token, requestId, {
      rating,
      review: review.trim() || selectedTags.join(', '),
      tags: selectedTags,
    });
    ...
    if (result.ok) {
      ...
      Alert.alert('Error', result.error || 'Could not submit rating. Please try again.');
    }
  };
      ...
      <SafeAreaView style={s.root}>
        ...
        <View style={s.successContainer}>
          ...
          <Text style={s.successSub}>
            Your {rating}-star review for {vendorName || 'the vendor'} has been submitted.
            This helps the community choose the best service providers.
          </Text>
          <TouchableOpacity style={s.doneBtn} onPress={() => navigation.goBack()}>
            <Text style={s.doneBtnText}>Done</Text>
          </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <View style={s.vendorCard}>
          ...
          <View>
            <Text style={s.vendorName}>{vendorName || 'Service Vendor'}</Text>
            <Text style={s.vendorCat}>{category || 'Maintenance'} Service</Text>
            {requestId && <Text style={{ fontSize: 11, color: P.muted, marginTop: 4 }}>Job #{requestId}</Text>}
          </View>
        </View>
        ...
        <View style={s.starSection}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {[1, 2, 3, 4, 5].map(i => (
              <TouchableOpacity key={i} onPress={() => setRating(i)} style={{ padding: 4 }} activeOpacity={0.8}>
                <Text style={{ fontSize: 44, color: i <= rating ? P.gold : '#CBD5E1' }}>★</Text>
              </TouchableOpacity>
            ))}
          </View>
          <Text style={{ fontSize: 16, fontWeight: '700', color: P.sub, marginTop: 12 }}>
            {rating === 0 ? 'Tap to rate' :
              rating === 1 ? '😞 Poor' :
                rating === 2 ? '😐 Fair' :
                  rating === 3 ? '🙂 Good' :
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
            {TAGS.map(tag => (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <Text style={[s.tagText, selectedTags.includes(tag) && { color: '#FFF' }]}>
                  {selectedTags.includes(tag) ? '✓ ' : ''}{tag}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={s.reviewSection}>
          ...
          <TextInput
            ...
            maxLength={500}
          />
          <Text style={{ fontSize: 11, color: P.muted, textAlign: 'right', marginTop: 6 }}>
            {review.length}/500
          </Text>
        </View>

        <TouchableOpacity
          style={[s.submitBtn, (rating === 0 || submitting) && { opacity: 0.5 }]}
          onPress={handleSubmit}
        ...
        <TouchableOpacity style={{ alignItems: 'center', paddingVertical: 10 }} onPress={() => navigation.goBack()}>
          <Text style={{ color: P.muted, fontSize: 14 }}>Skip for now</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  vendorCard: { flexDirection: 'row', alignItems: 'center', gap: 16, backgroundColor: P.surface, borderRadius: 18, padding: 18, marginBottom: 24, borderWidth: 1, borderColor: P.border },
  vendorIcon: { width: 60, height: 60, borderRadius: 30, backgroundColor: P.bg, alignItems: 'center', justifyContent: 'center' },
  vendorName: { fontSize: 17, fontWeight: '900', color: P.text },
  vendorCat: { fontSize: 13, color: P.sub, marginTop: 2 },
