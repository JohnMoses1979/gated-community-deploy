 ...
 *
 * SECURITY FIX (merged):
 *  - ALL appStore dependencies removed
 *  - Uses maintenanceStore + maintenanceApi (JWT auth, backend-driven)
 *  - residentId never sent in body — backend reads from JWT via CurrentUser.get()
 *  - Payment uses DemoPaymentModal (Razorpay WebView preserved in AmenityPaymentScreen)
...
import {
  CATEGORIES, PRIORITIES, TIME_SLOTS, CONTACT_OPTIONS,
  PRIORITY_COLORS, STATUS_STEPS,
          ...
          return (
            <View key={step.key} style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ alignItems: 'center', width: 60 }}>
                ...
                <Text style={[stp.label, active && { color: C.primary, fontWeight: '800' }, done && { color: '#0D6E6E' }]} numberOfLines={2}>
                  {step.label}
                </Text>
              </View>
              {i < STATUS_STEPS.length - 1 && (
...
const stp = StyleSheet.create({
  dot:      { width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center', backgroundColor: '#F0FAFA', borderWidth: 1.5, borderColor: '#B0DEDE', marginBottom: 4 },
  dotDone:  { backgroundColor: '#0D6E6E', borderColor: '#0D6E6E' },
  dotActive:{ backgroundColor: C.primary, borderColor: C.primary, transform: [{ scale: 1.15 }] },
  label:    { fontSize: 9, color: C.muted, textAlign: 'center', fontWeight: '600', maxWidth: 58 },
  line:     { width: 20, height: 2, backgroundColor: '#D0EEEE', marginBottom: 16 },
  lineDone: { backgroundColor: '#0D6E6E' },
});
...
// ─── Rate Modal ───────────────────────────────────────────────────────────────
function RateModal({ visible, onClose, req, token }) {
  const [rating, setRating]   = useState(0);
  const [comment, setComment] = useState('');
  const [loading, setLoading] = useState(false);
  const submitRating = useMaintenanceStore(s => s.submitRating);

  const handleSubmit = async () => {
    if (!rating) { Alert.alert('Please select a star rating'); return; }
    setLoading(true);
    const result = await submitRating(token, req.id, { rating, review: comment.trim(), tags: [] });
    setLoading(false);
    if (result.ok) {
      ...
      <View style={rm.overlay}>
        <View style={rm.sheet}>
          ...
          <TextInput
            ...
            textAlignVertical="top"
          />
          <TouchableOpacity style={[rm.btn, loading && { opacity: 0.7 }]} onPress={handleSubmit} disabled={loading}>
            {loading
              ? <ActivityIndicator color="#FFF" />
              : <Text style={rm.btnText}>⭐ Submit Review</Text>
            }
          </TouchableOpacity>
          <TouchableOpacity style={rm.cancel} onPress={onClose}>
            <Text style={rm.cancelText}>Skip</Text>
...
const rm = StyleSheet.create({
  ...
  sub:        { fontSize: 13, color: C.muted, marginBottom: 12 },
  input:      { backgroundColor: C.card, borderWidth: 1.5, borderColor: '#B0DEDE', borderRadius: 12, padding: 12, fontSize: 14, color: C.text, marginVertical: 10, minHeight: 80 },
  btn:        { backgroundColor: C.primary, borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginTop: 4 },
  btnText:    { color: '#FFF', fontWeight: '800', fontSize: 15 },
  cancel:     { paddingVertical: 14, alignItems: 'center' },
...
function RequestCard({ req, onPress }) {
  ...
  const needsAction = ['quote_sent_to_resident', 'payment_requested_to_resident', 'work_completed'].includes(req.status);

  return (
    <TouchableOpacity style={[sc.card, needsAction && { borderColor: '#FDE68A', borderWidth: 1.5 }]} onPress={onPress} activeOpacity={0.85}>
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 8, gap: 8 }}>
        <View style={{ flex: 1 }}>
          <Text style={sc.cardTitle} numberOfLines={1}>{req.title}</Text>
          <Text style={sc.cardSub}>{req.category} · {req.id}</Text>
        </View>
        <StatusBadge status={req.status} small />
      </View>

      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
        <View style={{ backgroundColor: pc.bg, paddingHorizontal: 10, paddingVertical: 3, borderRadius: 20 }}>
          <Text style={{ fontSize: 11, fontWeight: '800', color: pc.color }}>{req.priority}</Text>
        </View>
        <Text style={{ fontSize: 12, color: C.muted }}>
          {new Date(req.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
        </Text>
        {req.vendorName && (
          <View style={{ backgroundColor: '#E8F5F5', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 3, borderWidth: 1, borderColor: '#B0DEDE' }}>
            <Text style={{ fontSize: 11, fontWeight: '700', color: C.primary }}>🔧 {req.vendorName}</Text>
          </View>
        )}
      </View>

      {needsAction && (
        <View style={{ backgroundColor: C.warnBg, borderRadius: 8, padding: 8, marginTop: 4, borderWidth: 1, borderColor: '#FDE68A' }}>
          <Text style={{ fontSize: 12, fontWeight: '700', color: C.warn }}>
            {req.status === 'quote_sent_to_resident'          ? '💰 Quote ready — tap to review'
            : req.status === 'payment_requested_to_resident'  ? '💳 Payment due — tap to pay'
            : req.status === 'work_completed'                 ? '🔨 Work done — tap to approve'
            : ''}
          </Text>
        </View>
      )}
      <Text style={{ fontSize: 11, color: '#B0DEDE', textAlign: 'right', marginTop: 6, fontWeight: '600' }}>Tap to view →</Text>
    </TouchableOpacity>
  );
}

// ─── New Request Modal ────────────────────────────────────────────────────────
function NewRequestModal({ visible, onClose, onSubmit, submitting }) {
  const [form, setForm] = useState({
    title: '', description: '', category: 'Plumbing', priority: 'Medium',
    preferredSlot: 'Flexible', contactPref: 'Call',
  });
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
        <View style={{ backgroundColor: C.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, paddingTop: 20 }}>
          <TouchableOpacity onPress={onClose}><Text style={{ fontSize: 22, color: 'rgba(255,255,255,0.8)', fontWeight: '700' }}>✕</Text></TouchableOpacity>
          <Text style={{ fontSize: 18, fontWeight: '800', color: '#FFF' }}>New Request</Text>
          <View style={{ width: 32 }} />
        </View>
        <ScrollView contentContainerStyle={{ padding: 16 }} showsVerticalScrollIndicator={false}>
          <View style={{ backgroundColor: '#DBEAFE', borderRadius: 12, padding: 12, marginBottom: 16, borderWidth: 1, borderColor: '#BFDBFE' }}>
            <Text style={{ fontSize: 13, color: '#1E40AF', lineHeight: 18 }}>
              📋 Your request goes to admin, who assigns a vendor and shares a quote before work begins.
            </Text>
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}>
            {CATEGORIES.map(c => (
              <TouchableOpacity key={c} style={[nrm.chip, form.category === c && nrm.chipA]} onPress={() => f('category', c)}>
                <Text style={[nrm.chipT, form.category === c && { color: '#FFF' }]}>{c}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            {PRIORITIES.map(p => {
              ...
              const active = form.priority === p;
              return (
                <TouchableOpacity key={p} style={[nrm.chip, active && { backgroundColor: pc.bg, borderColor: pc.color }]} onPress={() => f('priority', p)}>
                  <Text style={[nrm.chipT, active && { color: pc.color, fontWeight: '800' }]}>{p}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
          ...
          <Text style={nrm.label}>Issue Title *</Text>
          <TextInput style={nrm.input} placeholder="e.g. Leaking pipe under kitchen sink" placeholderTextColor={C.muted} value={form.title} onChangeText={v => f('title', v)} />

          <Text style={nrm.label}>Description *</Text>
          <TextInput style={[nrm.input, { minHeight: 90, textAlignVertical: 'top' }]} placeholder="Describe the issue in detail…" placeholderTextColor={C.muted} value={form.description} onChangeText={v => f('description', v)} multiline />

          <Text style={nrm.label}>Preferred Time Slot</Text>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            {TIME_SLOTS.map(s => (
              <TouchableOpacity key={s} style={[nrm.chip, form.preferredSlot === s && nrm.chipA]} onPress={() => f('preferredSlot', s)}>
                <Text style={[nrm.chipT, form.preferredSlot === s && { color: '#FFF' }]}>{s}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            {CONTACT_OPTIONS.map(c => (
              <TouchableOpacity key={c} style={[nrm.chip, form.contactPref === c && nrm.chipA]} onPress={() => f('contactPref', c)}>
                <Text style={[nrm.chipT, form.contactPref === c && { color: '#FFF' }]}>{c}</Text>
              </TouchableOpacity>
            ))}
          </View>

          <TouchableOpacity
            style={[nrm.submitBtn, submitting && { opacity: 0.6 }]}
            onPress={() => {
              if (!form.title.trim() || !form.description.trim()) {
                Alert.alert('Missing Info', 'Please fill in the title and description.');
                return;
          ...
          >
            {submitting
              ? <ActivityIndicator color="#FFF" />
              : <Text style={{ color: '#FFF', fontSize: 15, fontWeight: '800' }}>🔧 Submit Request</Text>
            }
          </TouchableOpacity>
          <View style={{ height: 40 }} />
        </ScrollView>
...
const nrm = StyleSheet.create({
  label:     { fontSize: 12, fontWeight: '700', color: '#3D6E6E', marginBottom: 8, marginTop: 8 },
  input:     { backgroundColor: C.card, borderWidth: 1.5, borderColor: '#B0DEDE', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, color: C.text, marginBottom: 4 },
  chip:      { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, backgroundColor: C.card, borderWidth: 1.5, borderColor: '#B0DEDE', marginRight: 4 },
  chipA:     { backgroundColor: C.primary, borderColor: C.primary },
  chipT:     { fontSize: 13, fontWeight: '600', color: '#3D6E6E' },
  submitBtn: { backgroundColor: C.primary, borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 20 },
});

// ─── Detail Modal ─────────────────────────────────────────────────────────────
function DetailModal({ visible, req, onClose, token, onRateOpen }) {
  const [actionLoading, setActionLoading] = useState(false);
  const [showPayment, setShowPayment]     = useState(false);

  const respondToQuote        = useMaintenanceStore(s => s.respondToQuote);
  const approveCompletedWork  = useMaintenanceStore(s => s.approveCompletedWork);
  const simulatePayment       = useMaintenanceStore(s => s.simulatePayment);

  if (!req) return null;
  const pc = PRIORITY_COLORS[req.priority] || PRIORITY_COLORS.Medium;
  const isCompleted = ['closed'].includes(req.status);

  const handleRespondQuote = async (action) => {
    setActionLoading(true);
    const result = await respondToQuote(token, req.id, action);
    setActionLoading(false);
    if (result.ok) {
      Alert.alert(action === 'accept' ? '✅ Quote Accepted' : '❌ Quote Rejected',
        action === 'accept' ? 'Admin will confirm work start shortly.' : 'Admin will seek a revised quote.');
      onClose();
    } else {
      Alert.alert('Error', result.data?.message || 'Could not process. Please try again.');
    }
  };

  const handleApproveWork = async () => {
    setActionLoading(true);
    const result = await approveCompletedWork(token, req.id);
    setActionLoading(false);
    if (result.ok) {
      Alert.alert('✅ Work Approved', 'Vendor can now request payment.');
      onClose();
    } else {
  ...
  const handlePaymentSuccess = async (paymentData) => {
    ...
    setActionLoading(true);
    try {
      await simulatePayment(token, req.id, paymentData);
      Alert.alert('✅ Payment Successful', 'Your payment has been received. Job will be closed shortly.');
      onClose();
    } catch (e) {
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
        <View style={{ backgroundColor: C.primary, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 16, paddingTop: 20, gap: 10 }}>
          <TouchableOpacity onPress={onClose}><Text style={{ fontSize: 22, color: 'rgba(255,255,255,0.8)', fontWeight: '700', width: 32 }}>✕</Text></TouchableOpacity>
          <Text style={{ flex: 1, fontSize: 16, fontWeight: '800', color: '#FFF' }} numberOfLines={1}>{req.title}</Text>
        ...
        <ScrollView contentContainerStyle={{ padding: 14 }} showsVerticalScrollIndicator={false}>
          ...
          <View style={dm.card}>
            ...
            {[
              ...
              ['Category',   req.category],
              ['Priority',   req.priority],
              ['Submitted',  req.createdAt ? new Date(req.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'],
              req.preferredSlot && ['Preferred Slot', req.preferredSlot],
              req.contactPref   && ['Contact via',    req.contactPref],
              (req.assignedVendorName || req.vendorName) && ['Vendor', req.assignedVendorName || req.vendorName],
            ].filter(Boolean).map(([k, v]) => (
              <View key={k} style={dm.row}>
                <Text style={dm.rowKey}>{k}</Text>
                <Text style={dm.rowVal}>{v}</Text>
              </View>
            ))}
            <Text style={[dm.secLabel, { marginTop: 12 }]}>DESCRIPTION</Text>
            <Text style={{ fontSize: 14, color: '#3D6E6E', lineHeight: 22 }}>{req.description}</Text>
          </View>

          {/* Quote action */}
          {req.status === 'quote_sent_to_resident' && req.quote && (
            <View style={[dm.actionCard, { borderColor: '#FDE68A' }]}>
              <Text style={dm.actionTitle}>💰 Quote Received — Review & Decide</Text>
              <Text style={{ fontSize: 26, fontWeight: '900', color: '#0D6E6E', marginVertical: 6 }}>
                ₹{req.quote.amount?.toLocaleString('en-IN')}
              </Text>
              {req.quote.description && <Text style={{ fontSize: 14, color: '#3D6E6E' }}>{req.quote.description}</Text>}
              {req.quote.estimatedDays && <Text style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>📅 Est. {req.quote.estimatedDays} day(s)</Text>}
              <View style={{ flexDirection: 'row', gap: 10, marginTop: 12 }}>
                <TouchableOpacity
                  style={[dm.btn, { backgroundColor: C.primary, flex: 1 }, actionLoading && { opacity: 0.6 }]}
                  onPress={() => handleRespondQuote('accept')}
                  disabled={actionLoading}
                >
                  {actionLoading ? <ActivityIndicator color="#FFF" size="small" /> : <Text style={dm.btnT}>✓ Accept</Text>}
                </TouchableOpacity>
                <TouchableOpacity
                  style={[dm.btn, { backgroundColor: '#FEE2E2', flex: 1 }, actionLoading && { opacity: 0.6 }]}
                  onPress={() => Alert.alert('Reject Quote', 'Reject this quote?', [
                    { text: 'Cancel', style: 'cancel' },
                    { text: 'Reject', style: 'destructive', onPress: () => handleRespondQuote('reject') },
                  ])}
                  disabled={actionLoading}
                >
                  <Text style={[dm.btnT, { color: C.danger }]}>✕ Reject</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
          ...
          {/* Approve work */}
          {req.status === 'work_completed' && (
            <View style={[dm.actionCard, { borderColor: '#B0DEDE' }]}>
              <Text style={dm.actionTitle}>🔨 Work Completed — Please Inspect</Text>
              <Text style={{ fontSize: 13, color: '#5A8E8E', lineHeight: 19, marginBottom: 12 }}>
                Vendor has completed the work. Please inspect and approve to allow payment request.
              </Text>
              <TouchableOpacity
                style={[dm.btn, { backgroundColor: C.primary }, actionLoading && { opacity: 0.6 }]}
                onPress={handleApproveWork}
                disabled={actionLoading}
              >
                {actionLoading ? <ActivityIndicator color="#FFF" size="small" /> : <Text style={dm.btnT}>✓ Approve Work</Text>}
              </TouchableOpacity>
            </View>
          )}
          ...
          {req.status === 'payment_requested_to_resident' && (
            <View style={[dm.actionCard, { borderColor: '#FDE68A' }]}>
              <Text style={dm.actionTitle}>💳 Payment Due</Text>
              <Text style={{ fontSize: 26, fontWeight: '900', color: '#0D6E6E' }}>
                ₹{(req.finalAmount || req.quote?.amount)?.toLocaleString('en-IN')}
              </Text>
              <Text style={{ fontSize: 13, color: '#5A8E8E', marginTop: 4, marginBottom: 12 }}>
                Work is complete. Please pay to close this request.
              </Text>
              <TouchableOpacity
                style={[dm.btn, { backgroundColor: C.primary }, actionLoading && { opacity: 0.6 }]}
                ...
                disabled={actionLoading}
              >
                {actionLoading ? <ActivityIndicator color="#FFF" size="small" /> : <Text style={dm.btnT}>💳 Pay Now</Text>}
              </TouchableOpacity>
            </View>
          )}
          ...
          {isCompleted && (
            <View style={dm.card}>
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <TouchableOpacity style={[dm.btn, { backgroundColor: '#F59E0B', flex: 1 }]} onPress={() => { onClose(); onRateOpen(req); }}>
                  <Text style={dm.btnT}>⭐ Rate Job</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
              ...
              {[...req.timeline].reverse().map((t, i) => (
                <View key={i} style={{ flexDirection: 'row', gap: 10, paddingBottom: 14 }}>
                  <View style={{ alignItems: 'center', width: 14 }}>
                    <View style={{ width: 12, height: 12, borderRadius: 6, backgroundColor: i === 0 ? C.primary : '#B0DEDE', marginTop: 2 }} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 13, fontWeight: '700', color: C.text }}>{t.action}</Text>
                    {t.by  && <Text style={{ fontSize: 11, color: C.muted, marginTop: 1 }}>by {t.by}</Text>}
                    {t.at  && <Text style={{ fontSize: 11, color: C.muted }}>{new Date(t.at).toLocaleString('en-IN')}</Text>}
                  </View>
                </View>
              ))}
            </View>
      ...
      <DemoPaymentModal
        visible={showPayment}
        amount={req.finalAmount || req.quote?.amount || 0}
...
const dm = StyleSheet.create({
  card:       { backgroundColor: C.card, borderRadius: 16, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: '#D0EEEE' },
  secLabel:   { fontSize: 10, fontWeight: '800', color: C.muted, letterSpacing: 1, marginBottom: 10 },
  row:        { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 5, borderBottomWidth: 1, borderBottomColor: '#F0FAFA' },
  rowKey:     { fontSize: 13, color: C.muted },
  rowVal:     { fontSize: 13, fontWeight: '700', color: C.text, textAlign: 'right', flex: 1, marginLeft: 12 },
  actionCard: { backgroundColor: C.card, borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 2 },
  actionTitle:{ fontSize: 15, fontWeight: '800', color: C.text, marginBottom: 6 },
  btn:        { paddingVertical: 12, borderRadius: 12, alignItems: 'center', paddingHorizontal: 8 },
...
const ACTION_STATUSES   = ['quote_sent_to_resident', 'payment_requested_to_resident', 'work_completed'];
const PROGRESS_STATUSES = ['submitted', 'assigned', 'quoted', 'quote_accepted', 'approved_to_start', 'work_in_progress'];
const CLOSED_STATUSES   = ['resident_work_approved', 'payment_requested_to_admin', 'payment_received', 'closed', 'rejected', 'quote_rejected'];
...
export default function ResidentMaintenanceScreen({ navigation }) {
  ...
  const fetchMyRequests = useMaintenanceStore(s => s.fetchMyRequests);
  const submitRequest   = useMaintenanceStore(s => s.submitRequest);
  ...
  const filtered = sorted.filter(r => {
    if (filter === 'action')   return ACTION_STATUSES.includes(r.status);
    if (filter === 'progress') return PROGRESS_STATUSES.includes(r.status);
    if (filter === 'closed')   return CLOSED_STATUSES.includes(r.status);
    return true;
  });

  const actionCount = sorted.filter(r => ACTION_STATUSES.includes(r.status)).length;

  const handleSubmit = async (form) => {
    setSubmitting(true);
    try {
      const result = await submitRequest(token, form);
      setShowNew(false);
      Alert.alert(
        '✅ Request Submitted!',
        `Your ${form.category} request has been sent to admin.\n\nRequest ID: ${result.id}\n\nYou'll be notified once a vendor is assigned.`
      );
    } catch (e) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <View style={{ backgroundColor: C.primary, paddingTop: 40, paddingBottom: 16, paddingHorizontal: 20 }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View>
            ...
            <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.72)', marginTop: 1 }}>
              {requests.length} total{actionCount > 0 ? ` · ${actionCount} need action` : ''}
            </Text>
          </View>
          <TouchableOpacity
            style={{ backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1, borderColor: 'rgba(255,255,255,0.35)' }}
      ...
      <View style={{ flexDirection: 'row', backgroundColor: C.card, borderBottomWidth: 1, borderBottomColor: '#D0EEEE' }}>
        {FILTERS.map(({ k, l }) => (
          <TouchableOpacity
            key={k}
            style={{ flex: 1, paddingVertical: 11, alignItems: 'center', borderBottomWidth: filter === k ? 3 : 0, borderBottomColor: C.primary }}
            onPress={() => setFilter(k)}
          >
            <Text style={{ fontSize: 12, fontWeight: filter === k ? '800' : '600', color: filter === k ? C.primary : C.muted }}>
              {l}{k === 'action' && actionCount > 0 ? ` (${actionCount})` : ''}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {loading && !refreshing ? (
        ...
        <FlatList
          ...
          showsVerticalScrollIndicator={false}
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={() => loadRequests(true)} colors={[C.primary]} />
          }
          renderItem={({ item }) => (
            <RequestCard req={item} onPress={() => setSelectedReq(item)} />
          )}
          ListEmptyComponent={
            <View style={{ alignItems: 'center', paddingTop: 60, gap: 10, paddingHorizontal: 32 }}>
              ...
              <Text style={{ fontSize: 14, color: C.muted, textAlign: 'center', lineHeight: 20 }}>
                {filter === 'all' ? 'Tap "+ New" to submit a maintenance request.' : 'Switch to "All" to see all requests.'}
              </Text>
              {filter === 'all' && (
                <TouchableOpacity style={{ marginTop: 8, backgroundColor: C.primary, borderRadius: 14, paddingHorizontal: 24, paddingVertical: 13 }} onPress={() => setShowNew(true)}>
                  <Text style={{ color: '#FFF', fontWeight: '800', fontSize: 14 }}>+ New Request</Text>
                </TouchableOpacity>
              )}
            </View>
          }
      ...
      <DetailModal
        ...
        onClose={() => setSelectedReq(null)}
        token={token}
        onRateOpen={(req) => { setRateTarget(req); setShowRate(true); }}
      />
...
const sc = StyleSheet.create({
  card:      { backgroundColor: C.card, borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#D0EEEE', shadowColor: C.primary, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 2 },
