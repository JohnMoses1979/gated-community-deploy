...
const STATUS_META = {
  ...
  EXITED:   { label: '🚪 Vehicle Exited',     color: '#64748B', bg: '#F1F5F9' },
        ...
        <View style={styles.passCard}>
          ...
          <View style={styles.infoBox}>
            {[
              ...
              { label: 'Duration',      value: parking.durationDays || parking.duration ? `${parking.durationDays || parking.duration} day(s)` : '—' },
          ...
          {parking.status === 'ACTIVE' && parking.startTime && (
            <View style={[styles.statusBanner, { backgroundColor: '#EFF6FF', borderColor: '#1565C0', width: '100%', marginTop: 4 }]}>
              <Text style={{ color: '#1565C0', fontWeight: '700', fontSize: 13 }}>
                🚗 Guest entered at {fmt(parking.startTime)}
          ...
          {/* Expired detail */}
          {['EXITED', 'EXPIRED'].includes(parking.status) && parking.exitTime && (
