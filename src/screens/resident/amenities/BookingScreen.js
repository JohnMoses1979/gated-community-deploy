 ...
 *
 * SECURITY FIX (merged):
 *  - residentId, residentName, unit removed from all POST bodies
 *    (backend derives these from JWT via CurrentUser.get())
 *  - All API calls carry Authorization: Bearer <token>
 *  - /confirm-free and /create-order endpoints unchanged in path
 */
import React, { useState, useMemo } from "react";
import {
  View, Text, TouchableOpacity, StyleSheet, SafeAreaView, StatusBar,
...
export default function BookingScreen({ navigation, route }) {
  const theme = useTheme();
  const { amenity } = route.params || {};
  // FIX: token for Authorization header — no longer need user.id/name/unit for API calls
  const token = useAuthStore(s => s.token);
  const amenityBookings = useResidentStore(s => s.amenityBookings);
  const createAmenityBooking = useResidentStore(s => s.createAmenityBooking);
  const syncAmenityBooking = useResidentStore(s => s.syncAmenityBooking);
  // Still needed for local store duplicate-check (client-side UX only, server also validates)
  const user = useAuthStore(s => s.user);
  ...
  const handleBook = () => {
    ...
    // FIX: residentId, residentName, unit REMOVED from payload
    // Backend injects these from JWT via CurrentUser.get()
    ...
    if ((amenity.pricePerSlot || 0) === 0) {
      // FREE booking — POST /confirm-free
      fetch(apiUrl('/amenity-bookings/confirm-free'), {
        method: "POST",
        headers: authHeaders,
        ...
        .catch(err => {
          setLoading(false);
          Alert.alert("Error", err.message || "Could not confirm booking. Check connection.");
        });
    } else {
      // PAID — create local pending booking and go to payment screen
      setTimeout(() => {
        const booking = createAmenityBooking(bookingPayload);
        setLoading(false);
        navigation.replace("AmenityPayment", { booking });
      }, 500);
    }
