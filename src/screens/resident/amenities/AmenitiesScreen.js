 ...
 *
 * SECURITY FIX (merged):
 *  - GET /api/amenity-bookings/my  (was /resident/{id} — IDOR removed)
 *  - PATCH /api/amenity-bookings/{id}/cancel
 *  - All requests carry Authorization: Bearer <token> from JWT
 *  - residentId is never sent; backend derives it from JWT
 */
...
function BookingCard({ b, onPress, onCancel, cancelling }) {
  const statusColor =
    b.status === 'confirmed' ? C.success :
    b.status === 'cancelled' ? C.danger : C.warn;

  const today = new Date().toISOString().split('T')[0];
  const canCancel = b.status === 'confirmed' && !b.checkedIn && b.date >= today;
    ...
    >
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <View style={{ flex: 1 }}>
          ...
          {b.amount > 0 ? (
            <Text style={[st.cardS, {
              color: b.paymentStatus === 'paid' ? C.success : C.warn,
          ...
          {b.status === 'cancelled' && b.cancelledAt && (
            <Text style={[st.cardS, { color: C.danger, marginTop: 2 }]}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
        ...
        {canCancel && onCancel && (
          <TouchableOpacity
            style={[
              { backgroundColor: '#FEE2E2', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 10 },
...
export default function AmenitiesScreen({ navigation }) {
  ...
  const amenities = useResidentStore(s => s.amenities) ?? [];
  const amenityBookings = useResidentStore(s => s.amenityBookings) ?? [];
  // FIX: get token from auth store for Authorization header
  ...
  // ── Fetch bookings: /my endpoint — residentId derived from JWT on backend ──
  const fetchBookings = useCallback(() => {
    setLoadingBookings(true);
    fetch(apiUrl('/amenity-bookings/my'), {
      headers: {
      ...
      .then(r => r.json())
      .then(data => {
        setBackendBookings(Array.isArray(data) ? data : []);
  ...
  const upcoming = backendBookings
    .filter(b => b.status === 'confirmed' && !b.checkedIn && b.date >= today)
    .sort((a, b) => a.date.localeCompare(b.date));

  const pending = backendBookings
    .filter(b => b.status === 'payment_pending');

  const history = backendBookings
    .filter(b => b.checkedIn || b.status === 'cancelled' || b.date < today)
    .sort((a, b) => b.date.localeCompare(a.date));

  // ── Cancel: PATCH /amenity-bookings/{id}/cancel with JWT header ──────────
  const handleCancel = (booking) => {
    Alert.alert(
      'Cancel Booking',
      `Cancel ${booking.amenityName} on ${booking.date} at ${booking.slot}?\n\n` +
      (booking.paymentStatus === 'paid'
        ? 'You paid for this booking. Refunds are subject to society policy.'
        : 'This is a free booking and will be cancelled immediately.'),
      [
        { text: 'Keep Booking', style: 'cancel' },
        {
          text: 'Yes, Cancel',
          ...
          onPress: async () => {
            ...
            try {
              const res = await fetch(
                apiUrl(`/amenity-bookings/${booking.id}/cancel`),
                {
                  method: 'PATCH',
                  headers: {
                    'Authorization': `Bearer ${token}`,
              ...
              Alert.alert(
                '✅ Booking Cancelled',
                `Your ${booking.amenityName} booking has been cancelled successfully.`
              );
            } catch (err) {
    ...
    <SafeAreaView style={st.screen}>
      ...
      <View style={st.tabRow}>
        {[
          ...
          {
            key: 'upcoming',
            label: (upcoming.length + pending.length) > 0
              ? `Upcoming (${upcoming.length + pending.length})`
              : 'Upcoming',
          },
          {
            key: 'history',
            ...
            {amenities.length === 0 ? (
              <View style={st.empty}>
                ...
                <Text style={{ color: C.muted, fontSize: 13, textAlign: 'center', marginTop: 4 }}>
                  Contact admin to add amenities
                </Text>
              </View>
            ) : amenities.map(a => {
              const isOpen = a.available !== false && a.active !== false;
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
                    ...
                    <View style={{ alignItems: 'flex-end', gap: 6 }}>
                      <Badge
                        ...
                        color={isOpen ? C.success : C.muted}
                        bg={isOpen ? '#DCFCE7' : '#F1F5F9'}
                      />
                      {isOpen && <Text style={{ color: C.primary, fontSize: 20 }}>›</Text>}
                    </View>
                  </View>
