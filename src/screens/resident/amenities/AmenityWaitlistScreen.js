...
/**
 * AmenityWaitlistScreen.js
 *
 * Local waitlist UI — waitlist state is managed in residentStore.
 * No direct API calls in this screen (waitlist is client-side).
 * Token available if backend waitlist endpoint is added later.
 */
import React, { useState } from 'react';
import {
...
function WaitlistCard({ item, position, onRemove }) {
  return (
    <View style={[st.card, { borderLeftWidth: 4, borderLeftColor: C.accent }]}>
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <View style={st.posBadge}>
              <Text style={{ color: '#FFF', fontWeight: '900', fontSize: 13 }}>#{position}</Text>
            </View>
            <Text style={st.cardT}>{item.amenityEmoji || '🏛️'} {item.amenityName}</Text>
          </View>
          <Text style={st.cardS}>📅 {new Date(`${item.date}T00:00:00`).toLocaleDateString('en-IN', {
            weekday: 'short', day: '2-digit', month: 'short', year: 'numeric',
          })}</Text>
          <Text style={st.cardS}>⏰ {item.slot}  ·  👥 {item.members || 1} member(s)</Text>
          <Text style={[st.cardS, { color: C.muted, marginTop: 4, fontSize: 11 }]}>
...
export default function AmenityWaitlistScreen({ navigation }) {
  // token available for future backend waitlist API
  const token = useAuthStore(s => s.token);
  const user = useAuthStore(s => s.user);

  // waitlist stored locally in residentStore
  const waitlist = useResidentStore(s => s.amenityWaitlist) ?? [];
  const removeWaitlist = useResidentStore(s => s.removeAmenityWaitlist);

  const handleRemove = (item) => {
    Alert.alert(
      'Remove from Waitlist',
      `Remove waitlist entry for ${item.amenityName} on ${item.date} at ${item.slot}?`,
      [
  ...
  return (
    <SafeAreaView style={st.screen}>
      ...
      <View style={st.hdr}>
        ...
        <Text style={st.hdrT}>Amenity Waitlist</Text>
        <Text style={st.hdrSub}>
          {waitlist.length > 0
            ? `${waitlist.length} active waitlist entr${waitlist.length === 1 ? 'y' : 'ies'}`
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }} showsVerticalScrollIndicator={false}>
        ...
        {waitlist.length === 0 ? (
          <View style={st.empty}>
            ...
            <Text style={st.emptyT}>No Waitlist Entries</Text>
            <Text style={{ color: C.muted, fontSize: 13, textAlign: 'center', marginTop: 6, lineHeight: 20 }}>
              When a slot you want is full, you can join the waitlist.{'\n'}
              You'll be notified when a spot opens up.
            </Text>
            <TouchableOpacity
              style={[st.btn, { marginTop: 24, paddingHorizontal: 32 }]}
          ...
          <>
            <View style={[st.card, { backgroundColor: '#FEF3C7', borderColor: C.warn, borderWidth: 1.5, marginBottom: 16 }]}>
              <Text style={{ color: C.warn, fontWeight: '800', fontSize: 13, marginBottom: 4 }}>
                ⏳ How Waitlist Works
              </Text>
              <Text style={{ color: '#92400E', fontSize: 12, lineHeight: 18 }}>
                When a confirmed booking is cancelled, waitlisted residents are notified
                in order of position. Check the app regularly to confirm your slot.
              </Text>
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('Amenities')}
            >
              <Text style={st.btnT}>Browse More Amenities</Text>
            </TouchableOpacity>
          </>
        )}
...
const st = StyleSheet.create({
  ...
  cardT: { fontSize: 15, fontWeight: '800', color: C.text },
  cardS: { fontSize: 12, color: C.muted, marginTop: 3 },
  posBadge: { backgroundColor: C.accent, borderRadius: 10, paddingHorizontal: 8, paddingVertical: 3 },
  removeBtn: { backgroundColor: '#FEE2E2', paddingHorizontal: 12, paddingVertical: 7, borderRadius: 10 },
  empty: { alignItems: 'center', paddingVertical: 60, gap: 6 },
  emptyT: { fontSize: 18, fontWeight: '700', color: C.muted },
